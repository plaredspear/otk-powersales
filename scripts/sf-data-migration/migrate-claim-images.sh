...
#   클레임 이미지는 권한 통제 대상이라 private/ 저장 + presigned URL 로만 조회된다 (backend storage 레이어).
...
#   private/private/... 로 중복된다. unique_key = uploads/claim/migrated/{CV.Id}.{ext},
#   S3 실제 객체 key = private/ + unique_key. 신규 직접 등록(uploadClaimPhoto)도 동일하게 uploads/... 를 쓴다.
# --image-prefix 로 override 가능.
IMAGE_PREFIX="uploads/claim/migrated"
# S3 객체 key 는 private/ 하위에 위치한다 (presigned 조회 전용, anonymous GET 차단). 콘솔 업로드 대상 폴더 =
...
# 넣으면 private/private/... (또는 private/public/...) 로 어긋난다. (과거 사고 재발 방지)
...
if [[ "$prefix_lc" == private/* || "$prefix_lc" == public/* ]]; then
    ...
    echo "        예: uploads/claim/migrated. S3 객체 폴더는 PRIVATE_SEGMENT 가 자동 부여." >&2
    exit 1
fi
...
echo "           (콘솔에서 '$PRIVATE_SEGMENT/$IMAGE_PREFIX/' 폴더로 들어가 파일 업로드)"
echo "           ※ unique_key 는 segment 없이 '$IMAGE_PREFIX/...' 이고, S3 객체 key 는"
echo "             private/ 하위입니다 (backend 가 조회 시 private/ 합성 + presigned URL 발급)."
echo "           ※ 클레임 이미지는 권한 통제 대상 — private/ 는 anonymous read 차단 폴더여야 합니다."
echo
echo " ② Stage1 적재용 CSV"
echo "    로컬 : $UPLOAD_CSV"
