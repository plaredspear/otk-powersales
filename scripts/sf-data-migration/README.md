...
- **leader-erp-org-revoke substep** 은 조장(`6.조장`) 의 `object_permissions` 에서 다음 키를 제거해 해당 화면 접근을 닫는다 — `ERP_Order__c` / `ERP_OrderProduct__c` (ERP주문, 가드 `erp_order`), `Org__c` (조직마스터, 가드 `organization`), `DKRetail__CommuteLog__c` (근무 등록현황, 가드 `attendance_log`), `DKRetail__AlternativeHoliday__c` (대체휴무, 가드 `alternative_holiday`), `AttendInfo__c` (기준정보 > HR 적재 근무기간, 가드 `attend_info` — 근무기간 조회의 월별 근무내역 탭 지점/사원 셀렉터도 같은 가드라 함께 닫힌다), `DailySalesHistory__c` (기준정보 > ORORA 일매출, 가드 `daily_sales_history` — 이 키를 쓰는 endpoint 가 그 화면의 목록/거래처 lookup 2건뿐이라 파급이 화면 하나에 한정된다. 파급이 그 화면 하나에 한정된다), `MonthlySalesHistory__c` (가드 `monthly_sales_history` — 일매출과 달리 **기준정보 > ORORA 월매출 / 월별 진열사원 투입적합성 / 진열사원 배치 적합성 3화면이 이 키를 공유**해 함께 닫힌다. 각 화면의 전용 지점/거래처 셀렉터 포함. 매출/실적 대시보드 3화면은 `sales_dashboard` 로 분리되어 영향 없으며 leader-sales-dashboard-grant 가 담당한다). leader-profile-flags 가 JSON 전체를 SoT 로 덮어써 dirty row 를 skip 하는 것과 달리, 본 substep 은 **대상 키만 제거**하므로 `is_locally_modified=TRUE` 인 web admin 편집분에도 **강제 적용**하면서 다른 권한 편집은 보존한다 (사용자 결정). 공휴일(`HolidayMaster__c`) / 영업일(`WorkingDayMaster__c`) 은 조장 SoT 에 애초에 없어 회수 대상이 아니다. 멱등 (jsonb `-` 는 없는 키에 no-op). SoT 측에서도 동일 키가 제외되어 있어 신규 환경은 처음부터 권한이 부여되지 않는다. **web 화면 "조장 ERP주문/조직마스터 권한 회수 (6.조장)" 카드에서 실행**.
- **leader-sales-dashboard-grant substep** 은 조장(`6.조장`) 의 `custom_permissions` 에 `{"sales_dashboard": {"allowRead": true}}` 를 **병합**해 매출/실적 > 월 매출(물류배부) / 월 매출(전산실적) / POS매출 3화면과 거래처 상세의 매출 이력 탭을 연다. 3화면은 원래 적재 테이블 entity `monthly_sales_history` 로 가드됐으나 그 키가 기준정보 > ORORA 월매출 · 투입적합성 · 배치 적합성까지 함께 여닫아 화면 단위 통제가 불가능해 화면 전용 가상 자원(`SALES_DASHBOARD_RESOURCE`, `@PermissionResource`) 으로 분리했고, 분리는 승계 마이그레이션 없이 배포되므로 기존에 3화면을 보던 조장의 권한은 본 substep 으로만 복구된다. leader-erp-org-revoke 와 정확히 대칭 — **대상 키만 병합**(jsonb `||`)하므로 `is_locally_modified=TRUE` 인 web admin 편집분에도 **강제 적용**하면서 다른 권한 편집은 보존한다. 부여 대상은 `SfMigrationStage2Service.GRANTED_LEADER_CUSTOM_PERMISSIONS` 상수에서 관리하고, SoT (`LeaderProfileFlagsSeed` 의 `6.조장` custom_permissions) 에도 동일 키가 기재돼 신규 환경은 leader-profile-flags 로 같은 결과가 된다. 멱등 (`IS DISTINCT FROM` 가드). **web 화면 "조장 매출/실적 대시보드 권한 부여 (6.조장)" 카드에서 실행**.
