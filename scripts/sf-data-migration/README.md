...
| **Stage 2 변환** | `/admin/tools/sf-migration-2` | substep 별 Card 버튼. fk (비동기 + 진행률 바) → fk-natural-key → upload-file-polymorphic-parent → notice-rta-placeholder (dry-run/apply) → picklist (전체 / 개별 컬럼) → leader-profile-flags (조장 권한 SoT) → user-role-hierarchy. 최하단에 cut-over 최종 단계용 **Sharing Recalc** 카드 (최근 실행 이력 표시 + 전체 Recalc 실행). |
...
| **Stage 2-C** | BCrypt password hash (사번 기반 초기 평문 `{사번}@pwrs` → hash) | SF 데이터 마이그레이션 (Stage 2) | `POST /stage2/password` | 버튼 | substep 단위 |
| **Stage 2 최종** | 조장/지정 사번 비밀번호 **강제 초기화** (기존 값도 덮어씀) | SF 데이터 마이그레이션 (Stage 2) | `POST /stage2/leader-password-reset` | 버튼 (최하단) | substep 단위 |
...
| **Stage 2-H** | `branch_mapping` 누락 행 보정 (`BranchMappingSupplement` SoT — 현재 `E5694` CVS전략팀 1건, 확장값 `5691,5692,5693,5694,E5692,E5693,E5694`). SF `BranchMapping__mdt` 원본에 없어 Stage 1 CSV 로는 들어올 수 없는 행. 없으면 INSERT / 있으면 SoT 값과 다를 때 UPDATE(같으면 0 row, 멱등), Stage 1 적재와 순서 무관, 적용 시 `BranchCodeExpander` 캐시 재빌드(멀티 인스턴스는 롤링 재기동 필요) | Stage 2 "지점 코드 맵핑 보정 적재" | `POST /stage2/branch-mapping-supplement` | curl | substep 단위 |
...
**권장 실행 순서** (①·② 공통):
`fk` → `fk-natural-key` → `upload-file-polymorphic-parent` → `notice-rta-placeholder` (dry-run 확인 후 apply) → `picklist` (cost_center 동기화) → `password` (BCrypt) → `leader-profile-flags` (조장 권한 SoT) → `user-role-hierarchy` (snapshot 재계산). `fk` 는 비동기라 **DONE 확인 후** 다음 substep 을 호출한다.

> `leader-profile-flags` 는 `fk-natural-key` **이후**여야 한다 — 그 전에는 `profile_flags.profile_id` 가 NULL 이라 profile 조인이 전건 미해소되어 skip 만 보고된다.
...
- **Stage 2-C — 초기 비밀번호 적재 (BCrypt)** — "실행" 버튼 (동기). 대상 row 별 개별 encode 라 사원 수에 비례한 시간 소요. 초기 평문은 사번 기반 `{사번}@pwrs` + `password_change_required=TRUE`.
- **조장/지정 사번 비밀번호 초기화** (화면 최하단) — "실행" 버튼 (동기). 위 Stage 2-C 와 달리 `password IS NULL` 가드가 없어 **이미 설정된 비밀번호도 덮어쓴다**. `profile_id` 확정 이후 (fk → fk-natural-key → user-profile-reconcile) 1회 실행.
...
- **password substep** 은 `sfid IS NOT NULL AND (password IS NULL OR password = '')` 인 user row 의 password 를 **사번 기반 초기 평문 `"{사번}@pwrs"`** (`TemporaryPasswordPolicy.forEmployeeCode` — 사번이 없으면 `pwrs1234!` fallback) 로 BCrypt hash (backend 의 PasswordEncoder 빈 재사용, strength=10) + `password_change_required=TRUE` 설정. SF 는 비밀번호를 단방향 hash 로만 보관해 추출이 불가능하므로 **기존 비밀번호는 이전되지 않고 새로 발급**된다. 평문이 사번마다 다르므로 row 별로 개별 encode 한다. **web 화면 "Stage 2-C — 초기 비밀번호 적재 (BCrypt)" 카드에서 실행** (curl 도 가능).
- **leader-erp-org-revoke substep** 은 조장(`6.조장`) 의 `object_permissions` 에서 다음 키를 제거해 해당 화면 접근을 닫는다 — `ERP_Order__c` / `ERP_OrderProduct__c` (ERP주문, 가드 `erp_order`), `Org__c` (조직마스터, 가드 `organization`), `DKRetail__CommuteLog__c` (근무 등록현황, 가드 `attendance_log`), `DKRetail__AlternativeHoliday__c` (대체휴무, 가드 `alternative_holiday`), `AttendInfo__c` (기준정보 > HR 적재 근무기간, 가드 `attend_info` — 근무기간 조회의 월별 근무내역 탭 지점/사원 셀렉터도 같은 가드라 함께 닫힌다), `DailySalesHistory__c` (기준정보 > ORORA 일매출, 가드 `daily_sales_history` — 이 키를 쓰는 endpoint 가 그 화면의 목록/거래처 lookup 2건뿐이라 파급이 화면 하나에 한정된다. 파급이 그 화면 하나에 한정된다), `MonthlySalesHistory__c` (가드 `monthly_sales_history` — 일매출과 마찬가지로 파급이 **기준정보 > ORORA 월매출 1화면**에 한정된다. 목록 + 전용 거래처 lookup 포함. 이 키는 원래 5화면을 함께 여닫았으나 두 차례 자원 분리로 매출/실적 대시보드 3화면(`sales_dashboard`) 과 진열사원 적합성 2화면(`display_employee_adequacy`) 이 빠져나갔고, 둘 다 leader-sales-dashboard-grant 가 별도 부여한다). leader-profile-flags 가 JSON 전체를 SoT 로 덮어써 dirty row 를 skip 하는 것과 달리, 본 substep 은 **대상 키만 제거**하므로 `is_locally_modified=TRUE` 인 web admin 편집분에도 **강제 적용**하면서 다른 권한 편집은 보존한다 (사용자 결정). 공휴일(`HolidayMaster__c`) / 영업일(`WorkingDayMaster__c`) 은 조장 SoT 에 애초에 없어 회수 대상이 아니다. 멱등 (jsonb `-` 는 없는 키에 no-op). SoT 측에서도 동일 키가 제외되어 있어 신규 환경은 처음부터 권한이 부여되지 않는다. **web 화면 "조장 ERP주문/조직마스터 권한 회수 (6.조장)" 카드에서 실행**.
- **leader-sales-dashboard-grant substep** 은 조장(`6.조장`) 의 `custom_permissions` 에 `{"sales_dashboard": {"allowRead": true}, "display_employee_adequacy": {"allowRead": true}}` 를 **병합**해 아래 5화면을 연다 (둘 다 조회 전용이라 READ 단독):
  - `sales_dashboard` (`SALES_DASHBOARD_RESOURCE`) — 매출/실적 > 월 매출(물류배부) / 월 매출(전산실적) / POS매출 3화면 + 거래처 상세의 매출 이력 탭.
  - `display_employee_adequacy` (`DISPLAY_EMPLOYEE_ADEQUACY_RESOURCE`) — 여사원 일정 > 월별 진열사원 투입적합성 / 진열사원 배치 적합성 2화면. **각 화면 전용 지점 셀렉터**(`/sales/input-adequacy/branches` · `/sales/deployment/branches`) 도 같은 자원으로 가드되어 함께 열린다 — 셀렉터를 옛 키에 남기면 메뉴는 보이는데 지점 목록만 403 이 된다.

  5화면 모두 원래 적재 테이블 entity `monthly_sales_history` 로 가드됐으나 그 키가 기준정보 > ORORA 월매출까지 함께 여닫아 화면 단위 통제가 불가능해 두 차례에 걸쳐 화면 전용 가상 자원(`@PermissionResource`) 으로 분리했고, 분리는 승계 마이그레이션 없이 배포되므로 기존에 그 화면들을 보던 조장의 권한은 본 substep 으로만 복구된다 (ORORA 월매출은 분리 대상이 아니라 계속 회수 상태). leader-erp-org-revoke 와 정확히 대칭 — **대상 키만 병합**(jsonb `||`)하므로 `is_locally_modified=TRUE` 인 web admin 편집분에도 **강제 적용**하면서 다른 권한 편집은 보존한다. 부여 대상은 `SfMigrationStage2Service.GRANTED_LEADER_CUSTOM_PERMISSIONS` 상수에서 관리하며 **JSON 을 통째로 병합**하므로 자원이 늘어도 상수에 키만 더하면 된다 (substep 이름은 최초 도입 자원을 따라 유지). SoT (`LeaderProfileFlagsSeed` 의 `6.조장` custom_permissions) 에도 동일 키가 기재돼 신규 환경은 leader-profile-flags 로 같은 결과가 된다. 멱등 (`IS DISTINCT FROM` 가드). **web 화면 "조장 화면 전용 자원 권한 부여 (6.조장)" 카드에서 실행**.
- **leader-password-reset substep** 은 profile `6.조장` 인 user + `SfMigrationStage2Service.MANUAL_PASSWORD_RESET_EMPLOYEE_CODES` 에 열거된 지정 사번 user 의 password 를 동일 규칙(`{사번}@pwrs`)으로 **강제 초기화**한다. password substep 과 달리 **`password IS NULL` 가드가 없어 이미 설정된 비밀번호도 덮어쓴다** — 재실행하면 대상자가 바꾼 비밀번호도 매번 초기값으로 되돌아가므로 cut-over 시 1회 실행하는 운영 도구다. `7.영업사원 + 조장` 은 대상 밖 (leader-profile-flags 와 동일 범위). 신규 DB 에 없는 지정 사번은 응답 results 에 "미존재 사번" 행으로 보고된다. **profile_id 확정 이후 실행 의무** — 그 전에는 조장 대상이 0건으로 잡힌다. **web 화면 최하단 "조장/지정 사번 비밀번호 초기화" 카드에서 실행**.
...
6. **Sharing recalc** (spec #792) — OWD / Record Type / FLS / Sharing Rule 등 모든 sharing 관련 cache 일괄 무효화. web `SF Migration Stage 2` 화면 최하단 **"Sharing Recalc (cut-over 최종 단계)"** 카드의 "전체 Recalc 실행" 버튼으로 처리한다 (최근 실행 이력도 함께 표시). curl 대안:
   ...
   ```
   > ⚠️ 본 엔드포인트는 Stage 1·2 substep 과 달리 **`MODIFY_ALL_DATA` 권한 가드**가 걸려 있다 (`SharingRecalcAdminController`). 시스템 관리자 Profile 이 연결된 계정으로 실행해야 하며, `user.profile_id` 가 NULL 인 계정 (예: SF Profile 적재 전에 생성된 prod 부트스트랩 계정) 은 403 이 난다 — `SfPermissionResolver.isSystemAdmin` 이 `user.profileId → profile.name` 으로 판정하기 때문. 403 시 해당 계정의 Profile 연결을 먼저 확인한다.
...
| 파일 | 책임 |
|---|---|
| `migrate-claim-images.sh` | 단일 진입점 오케스트레이션 (query → download → build-csv + 콘솔 S3 업로드 경로 안내). **로컬 산출물 준비까지만 담당** — S3 업로드는 콘솔 수동, Stage1/Stage2 는 web 화면에서 처리. 각 단계 멱등 / `--skip-*` 부분 재시도 |
| `build-claim-upload-files.main.kts` | ContentDocumentLink 메타 CSV → `claim_upload_files.csv` 변환 (오프라인, AWS 미사용). 이미지 필터 + sfid dedup. 단독 실행 가능 |

> **bucket 인자 없음**: 본 스크립트는 S3 bucket 을 다루지 않는다. Stage1 적재 시 bucket 은 **web SF Migration Stage 1 화면이 backend `app.aws.s3.bucket` 프리필로 자동 표시**한다. 스크립트가 받는 옵션은 `--org / --api-version / --image-prefix / --image-exts / --stage1-prefix / --out-dir / --skip-query / --skip-download / --skip-build-csv / --count-only / --limit / --parallel` 뿐 (`--bucket / --aws-upload / --trigger / --api-base / --token` 은 존재하지 않으며, 붙이면 `Unknown arg` 에러).
...
3) build-csv  build-claim-upload-files.main.kts → <out>/claim_upload_files.csv (이미지 필터 + dedup)
--- 여기까지가 스크립트 담당 (로컬 산출물 준비). 이후는 콘솔/web 수동 ---
S3 업로드   AWS 콘솔에서 직접 (스크립트가 경로 안내). <out> = input/claim-images/
web Stage1  SF Migration Stage 1 화면 "클레임 이미지" 카드 (target=ClaimImageUploadFile)
web Stage2  upload-file-polymorphic-parent substep
```

**S3 업로드는 AWS 콘솔에서 사용자가 직접 수행** (스크립트는 업로드 대상 두 곳을 안내만). bucket 은 web Stage 1 폼 프리필 값과 동일한 운영 S3 버킷:
- `<out>/images/` 의 파일들 → `s3://<bucket>/private/uploads/claim/migrated/` (실제 S3 객체 key = `private/` + unique_key. `--image-prefix` 로 `uploads/claim/migrated` 부분 override 가능)
- `<out>/claim_upload_files.csv` → `s3://<bucket>/<stage1-prefix>/claim_upload_files.csv` (기본 stage1-prefix = `sf-migration/claim-images`)

**이미지 파일명(`{ContentVersion.Id}.{ext}`)을 바꾸지 말 것** — `claim_upload_files.csv` 의 `UniqueKey__c` 와 1:1 로 맞물린다.

핵심 매핑 (한 행/ContentDocumentLink → `claim_upload_files.csv`):

| claim_upload_files.csv (SF 필드명) | 값 | upload_file 컬럼 |
|---|---|---|
...
# 기본 — 로컬 준비(query+download+build-csv) 후 콘솔 업로드 경로 안내. bucket 인자 없음.
./migrate-claim-images.sh --org <alias>
#   → 출력 안내대로 AWS 콘솔에서 (bucket = web Stage1 폼 프리필 값):
#       ① <out>/images/* → s3://<bucket>/private/uploads/claim/migrated/
#       ② <out>/claim_upload_files.csv → s3://<bucket>/sf-migration/claim-images/claim_upload_files.csv
#   → 그 후 web SF Migration Stage1(클레임 이미지 카드) → Stage2(upload-file-polymorphic-parent)

# 추출 대상 건수만 미리 확인 (다운로드 안 함)
./migrate-claim-images.sh --org <alias> --count-only

# 샘플 100건만 추출 (검증용)
./migrate-claim-images.sh --org <alias> --limit 100

# 변환(build-csv)만 재시도 (이미 다운로드 끝났을 때)
./migrate-claim-images.sh --org <alias> --skip-query --skip-download

# 이미지 확장자를 레거시 UI 허용분으로 좁히기 (기본은 여유 화이트리스트)
./migrate-claim-images.sh --org <alias> --image-exts jpg,jpeg,png

# 다운로드 병렬 (수십시간 → 수시간). SF API rate limit 내에서 8~16 권장
./migrate-claim-images.sh --org <alias> --parallel 12

# Stage1 CSV prefix override (기본 sf-migration/claim-images)
./migrate-claim-images.sh --org <alias> --stage1-prefix <prefix>
```
