...
3) s3-images  aws s3 cp <out>/bin/ → s3://<bucket>/uploads/claim/migrated/
4) build-csv  build-claim-upload-files.main.kts → <out>/upload_files.csv
5) upload-csv aws s3 cp upload_files.csv → s3://<bucket>/<stage1-prefix>/upload_files.csv
6) trigger    (옵션) Stage1 copy-from-s3(UploadFile) → polling → Stage2 upload-file-polymorphic-parent
```
...
```bash
# 사전: sf org login web --alias <alias>,  AWS 자격증명 설정 (aws configure / AWS_PROFILE)

# 전 단계 일괄 (Stage1/2 는 web 화면에서 수동)
./migrate-claim-images.sh --org <alias> --bucket <s3-bucket> --stage1-prefix <prefix>

# Stage1/Stage2 까지 자동 (backend 토큰 필요)
./migrate-claim-images.sh --org <alias> --bucket <s3-bucket> --stage1-prefix <prefix> \
    --trigger --api-base https://<host> --token <JWT>

# 변환/업로드만 재시도 (이미 다운로드 끝났을 때)
./migrate-claim-images.sh --org <alias> --bucket <s3-bucket> --stage1-prefix <prefix> \
