...
echo
echo " ④ web SF Migration 화면 '공지 본문 이미지 placeholder 치환' 섹션 (Stage1/Stage2 완료 후):"
echo "    · 본문 HTML 의 rtaImage <img> → placeholder(notice-image://{refid} + data-refid) 로 치환 (notice.contents UPDATE)."
echo "    · backend 가 DB 에서 직접 정규식 매칭하므로 scan CSV / 로컬 kotlinc 불요."
echo "    · 먼저 'Dry-run' 으로 변경 규모 확인 → 이상 없으면 'Apply' 로 실제 UPDATE (비가역, 멱등)."
echo "    · presigned URL 은 만료되므로 본문엔 placeholder 만 저장 — 조회 시 backend 가 presigned 로 rewrite."
echo "    (로컬 대안: replace-notice-rta-urls.main.kts — db.properties 필요. web 화면을 우선 사용.)"
echo
