...
| 7 | **조장 화면 권한 회수** | `6.조장` 에서 ERP주문(`erp_order`) / 조직마스터(`organization`) / 근무 등록현황(`attendance_log`) / 대체휴무(`alternative_holiday`) / HR 적재 근무기간(`attend_info`) / ORORA 일매출(`daily_sales_history`) / ORORA 월매출(`monthly_sales_history`) 권한 키 제거. 매출/실적 대시보드 3화면(`sales_dashboard`) 과 진열사원 적합성 2화면(`display_employee_adequacy`) 은 별도 자원이라 영향 없음. **dirty row 에도 강제 적용**(대상 키만 제거라 다른 편집분 보존) (`POST .../stage2/leader-erp-org-revoke`) |
| 8 | **조장 화면 전용 자원 권한 부여** | `6.조장` 의 `custom_permissions` 에 `sales_dashboard: allowRead` + `display_employee_adequacy: allowRead` 병합 → ① 월 매출(물류배부/전산실적)·POS매출 3화면 + 거래처 상세 매출 이력 탭 ② 월별 진열사원 투입적합성·진열사원 배치 적합성 2화면(각 화면 전용 지점 셀렉터 포함) 개방. 두 자원 모두 `monthly_sales_history` 가드에서 화면 전용 가상 자원으로 분리하면서 승계 없이 배포했으므로 본 substep 이 조장 권한을 복구한다. **dirty row 에도 강제 적용**(대상 키만 병합이라 다른 편집분 보존) (`POST .../stage2/leader-sales-dashboard-grant`) |
| 9 | **지점 코드 맵핑 보정 적재** | SF `BranchMapping__mdt` 원본에 빠져 있는 `branch_mapping` 행을 채운다 (현재 `E5694` CVS전략팀 1건). CVS전략팀은 조직코드가 `E5694` 인데 SF 매핑 키가 평문 `5694` 라 지점 코드 확장이 일어나지 않아 해당 조장의 거래처·여사원 일정 조회가 0건이 된다. 기존 `5694` 행은 전사 권한자 34개 고정 목록이 쓰므로 **지우지 않고 행을 추가**한다. 확장값은 `5691,5692,5693,5694,E5692,E5693,E5694` — 운영 CVS 거래처가 전량 E 코드로 적재돼 있어(`E5692` 706건 / `E5693` 440건, 평문 0건) SF 원본의 평문 값만으로는 여전히 0건이다. 없으면 INSERT / 있으면 backend SoT(`BranchMappingSupplement`) 값과 다를 때 UPDATE (같으면 0 row, 멱등). Stage 1 적재와 순서 무관, 적용 시 확장 캐시 재빌드 — **멀티 인스턴스면 롤링 재기동 필요**(JVM 로컬 캐시) (`POST .../stage2/branch-mapping-supplement`) |
