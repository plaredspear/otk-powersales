...
| 9 | **지점 코드 맵핑 보정 적재** | SF `BranchMapping__mdt` 원본에 빠져 있는 `branch_mapping` 행을 채운다 (현재 `E5694` CVS전략팀 1건). CVS전략팀은 조직코드가 `E5694` 인데 SF 매핑 키가 평문 `5694` 라 지점 코드 확장이 일어나지 않아 해당 조장의 거래처·여사원 일정 조회가 0건이 된다. 기존 `5694` 행은 전사 권한자 34개 고정 목록이 쓰므로 **지우지 않고 행을 추가**한다. Stage 1 `BranchMapping` 적재와 순서 무관 + 기존 행 보존(멱등), 적재 시 확장 캐시 재빌드 (`POST .../stage2/branch-mapping-supplement`) |
