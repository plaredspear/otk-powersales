...
| 7 | **조장 화면 권한 회수** | `6.조장` 에서 ERP주문(`erp_order`) / 조직마스터(`organization`) / 근무 등록현황(`attendance_log`) / 대체휴무(`alternative_holiday`) / HR 적재 근무기간(`attend_info`) 권한 키 제거. **dirty row 에도 강제 적용**(대상 키만 제거라 다른 편집분 보존) (`POST .../stage2/leader-erp-org-revoke`) |
