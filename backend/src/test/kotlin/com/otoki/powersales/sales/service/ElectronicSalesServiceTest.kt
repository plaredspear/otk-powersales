...
class ElectronicSalesServiceTest {
	...
	private fun row(itemCd: String, itemNm: String?, amt: Long, qty: Long): ElectronicSalesRow {
		val r = mockk<ElectronicSalesRow>()
	...
	fun querySourceAndMapping() {
		...
		every {
			liveTotSalesDailyRepository.aggregateByProduct(capture(custSlot), capture(startSlot), capture(endSlot))
		} returns listOf(
			row("01101123", "갈릭 아이올리소스 240g", 3500, 10),
			row("01101222", "오뚜기 3분 카레 100g", 1500, 5),
		)

		val res = service.getElectronicSales(1, "202602")
	...
	fun accountNotFound() {
		...
		assertThatThrownBy { service.getElectronicSales(99, "202602") }
	...
	fun noExternalKey() {
		...
		verify(exactly = 0) { liveTotSalesDailyRepository.aggregateByProduct(any(), any(), any()) }
	}
	...
	fun posDownGracefulFallback() {
		...
		every { liveTotSalesDailyRepository.aggregateByProduct(any(), any(), any()) } throws RuntimeException("POS down")
