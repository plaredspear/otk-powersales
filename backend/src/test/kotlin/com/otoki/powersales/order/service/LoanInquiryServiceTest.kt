...
class LoanInquiryServiceTest {
    ...
    private val service = LoanInquiryService(loanInquirySender)
    ...
    fun success() {
        every { loanInquirySender.inquire(eq(externalKey)) } returns LoanInquirySapResult(
    ...
    fun sapErrorPropagates() {
        every { loanInquirySender.inquire(eq(externalKey)) } throws LoanSapErrorException("거래처 미존재")
    ...
    fun htmlResponsePropagates() {
        every { loanInquirySender.inquire(eq(externalKey)) } throws LoanSapHtmlResponseException()
    ...
    fun unavailablePropagates() {
        every { loanInquirySender.inquire(eq(externalKey)) } throws LoanSapUnavailableException("SAP 네트워크 오류")
