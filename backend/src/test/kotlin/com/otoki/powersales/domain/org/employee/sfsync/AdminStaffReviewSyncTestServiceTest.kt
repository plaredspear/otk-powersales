...
@DisplayName("AdminStaffReviewSyncTestService (SF IF_SendStaffReviewToPWS 조회 테스트 도구) 테스트")
class AdminStaffReviewSyncTestServiceTest {

    private lateinit var sfOutboundClient: SfOutboundClient
    private lateinit var service: AdminStaffReviewSyncTestService

    @BeforeEach
    fun setUp() {
        sfOutboundClient = mockk()
        service = AdminStaffReviewSyncTestService(sfOutboundClient, ObjectMapper())
    }

    private fun request(modDt: String = "20260410") =
        AdminStaffReviewSyncTestRequest(modDt = modDt)

    @Test
    @DisplayName("MOD_DT 를 /IF_SendStaffReviewToPWS 로 POST — 응답 rawBody 그대로 노출")
    fun test_callsSfWithModDt() {
        val rawList = """[{"EmployeeNumber":"E-1","EmployeeTotalScore":"25"}]"""
