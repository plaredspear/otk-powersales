...
import com.otoki.powersales.domain.org.leave.exception.AltHolidayConfirmDateIsHolidayException
...
class AlternativeHolidayServiceTest {
    ...
    private val service = AlternativeHolidayService(
    ...
    inner class CreateTests {
        ...
        fun create_duplicate() {
            every { employeeRepository.findById(1L) } returns Optional.of(createEmployee())
            every { validator.validateNoDuplicate(1L, saturday) } throws AltHolidayDuplicateException()
    ...
    inner class GetTests {
        ...
        fun get_withDateRange() {
            ...
            every {
                alternativeHolidayRepository.findByEmployeeIdAndActualWorkDateBetweenOrderByCreatedAtDesc(
                    1L, LocalDate.of(2026, 1, 1), LocalDate.of(2026, 3, 31)
                )
            } returns listOf(altHoliday)
        ...
        fun get_defaultDateRange() {
            ...
            every { employeeRepository.findById(1L) } returns Optional.of(employee)
            every {
                alternativeHolidayRepository.findByEmployeeIdAndActualWorkDateBetweenOrderByCreatedAtDesc(1L, any(), any())
            } returns emptyList()
