...
@DisplayName("AccountUpdateTxService 테스트 (거래처 수정 검증/갱신 트랜잭션, Spec #643 P1-B)")
class AccountUpdateTxServiceTest {
    ...
    inner class HappyPathAndPartialUpdate {
        ...
        @Test
        @DisplayName("T1 정상 수정 - name + address1 + phone 동시 갱신 + 주소 변경 true 반환")
        fun t1_success() {
            ...
            val addressChanged = service.applyUpdate(
                id = accountId,
            ...
            assertThat(account.name).isEqualTo("(신규) 강남점 수정")
            assertThat(account.address1).isEqualTo("서울특별시 강남구 테헤란로 100")
            assertThat(account.phone).isEqualTo("02-1234-5678")
        }

        @Test
        @DisplayName("T2 빈 페이로드 PUT (모든 필드 null) - 변경 0건 + 주소 변경 false")
        fun t2_emptyPayload() {
        ...
        @Test
        @DisplayName("T3 name 만 포함 - name 만 갱신 + 다른 필드 보존 + 주소 변경 false")
        fun t3_nameOnly() {
            ...
            val addressChanged = service.applyUpdate(
                id = accountId,
                principal = branchManagerPrincipal,
                request = AdminAccountUpdateRequest(name = "(신규) 강남점 수정")
            )
    ...
    @Nested
    @DisplayName("주소 변경 감지 (좌표 재조회 트리거 판정)")
    inner class AddressChangeDetection {

        @Test
        @DisplayName("T20 address1 변경 - addressChanged true 반환")
        fun t20_address1Changed_returnsTrue() {
            val account = nativeAccount(address1 = "기존 주소1")
            every { accountRepository.findActiveById(accountId) } returns account
        ...
        @Test
        @DisplayName("T23 address1 동일 값 재전송 - 변경 아님 → addressChanged false 반환")
        fun t23_address1SameValue_returnsFalse() {
            val account = nativeAccount(address1 = "기존 주소1")
            every { accountRepository.findActiveById(accountId) } returns account
    ...
    @Nested
    @DisplayName("findResponse - 갱신 후 응답 조회")
    inner class FindResponse {

        @Test
        @DisplayName("T26 갱신 후 entity 매핑 응답 반환")
        fun t26_findResponse() {
            val account = nativeAccount(name = "(신규) 강남점", address1 = "서울특별시 강남구")
            every { accountRepository.findActiveById(accountId) } returns account

            val response = service.findResponse(accountId)

            assertThat(response.id).isEqualTo(accountId)
            assertThat(response.name).isEqualTo("(신규) 강남점")
            assertThat(response.address1).isEqualTo("서울특별시 강남구")
        }

        @Test
        @DisplayName("T27 findResponse - 비존재 id → ACCOUNT_NOT_FOUND")
        fun t27_findResponseNotFound() {
            every { accountRepository.findActiveById(9999) } returns null
