...
import com.otoki.powersales.domain.activity.schedule.exception.TeamScheduleAccountRequiredException
...
import com.otoki.powersales.domain.activity.schedule.exception.TeamScheduleInvalidRangeException
...
import com.otoki.powersales.domain.activity.schedule.exception.TeamSchedulePastDateChangeException
import com.otoki.powersales.domain.activity.schedule.exception.TeamScheduleRangeTooWideException
...
import com.otoki.powersales.domain.activity.schedule.service.TeamScheduleValidator
...
class AdminTeamScheduleServiceTest {
    ...
    private fun dummyDisplayMaster(): DisplayWorkSchedule =
        DisplayWorkSchedule(id = 9999L)
    ...
    inner class GetMembersTests {
        ...
        fun getMembers_leader_singleCostCenter() {
            ...
            val m1 = createEmployee(id = 2L, employeeCode = "20030002", name = "김영희", role = AppAuthority.WOMAN, orgName = "강북유통지점", jikwee = "사원", status = "재직")
    ...
    inner class GetSchedulesWithSummaryTests {
        ...
        fun getSchedulesWithSummary_rangeTooWide() {
            ...
            assertThrows<TeamScheduleRangeTooWideException> {
                service.getSchedulesWithSummary(from, to, listOf(1L), null)
            }
            verify { teamMemberScheduleRepository wasNot Called }
        }
        ...
        fun getSchedulesWithSummary_invalidRange() {
            ...
            assertThrows<TeamScheduleInvalidRangeException> {
                service.getSchedulesWithSummary(from, to, listOf(1L), null)
            }
            verify { teamMemberScheduleRepository wasNot Called }
        }
    ...
    inner class UpdateScheduleTests {
        ...
        fun updateSchedule_pastDate_displaySchedule_dateChange() {
            ...
            val request = TeamScheduleUpdateRequest(
                workingDate = LocalDate.now().plus(1, ChronoUnit.DAYS).toString(),
                workingType = WorkingType.WORK,
                workingCategory1 = WorkingCategory1.DISPLAY,
                workingCategory3 = WorkingCategory3.FIXED,
                accountId = 1
        ...
        fun updateSchedule_pastDate_nullCategory_dateChange() {
            // Given
            val currentUser = createEmployee(id = 10L, role = AppAuthority.LEADER)
            val threeDaysAgo = LocalDate.now().minus(3, ChronoUnit.DAYS)
            val schedule = createSchedule(
                id = 100L,
                employeeId = 1L,
        ...
        fun updateSchedule_futureDate_dateChange_success() {
            ...
            val currentUser = createEmployee(id = 10L, role = AppAuthority.LEADER)
            val tomorrow = LocalDate.now().plus(1, ChronoUnit.DAYS)
            val dayAfter = LocalDate.now().plus(2, ChronoUnit.DAYS)
            val schedule = createSchedule(
                id = 100L,
                employeeId = 1L,
    ...
    inner class MassDeleteTests {
        ...
        fun massDelete_success() {
            ...
            val s100 = createSchedule(id = 100L, employeeId = 11L, accountId = 21, workingDate = LocalDate.of(2026, 5, 10))
            val s101 = createSchedule(id = 101L, employeeId = 11L, accountId = 21, workingDate = LocalDate.of(2026, 5, 15))
            // 다른 그룹 (employee=12, account=22, YearMonth=2026-05) → refresh 1회
            val s102 = createSchedule(id = 102L, employeeId = 12L, accountId = 22, workingDate = LocalDate.of(2026, 5, 20))
        ...
        fun massDelete_rowLimitExceeded() {
            ...
            assertThatThrownBy { service.massDelete(principalOf(leader), ids) }
                .isInstanceOf(TeamScheduleMassDeleteRowLimitExceededException::class.java)
            verify(exactly = 0) { teamMemberScheduleRepository.findAllById(any<List<Long>>()) }
        }
        ...
        fun massDelete_notFoundPartial() {
            ...
            assertThatThrownBy { service.massDelete(principalOf(leader), listOf(100L, 101L)) }
                .isInstanceOfSatisfying(TeamScheduleNotFoundPartialException::class.java) {
