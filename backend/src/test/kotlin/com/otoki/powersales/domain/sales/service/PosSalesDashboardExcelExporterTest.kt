...
@DisplayName("PosSalesDashboardExcelExporter 테스트")
class PosSalesDashboardExcelExporterTest {

    private val exporter = PosSalesDashboardExcelExporter()

    private val start = LocalDate.of(2026, 5, 1)
    private val end = LocalDate.of(2026, 5, 31)

    private fun item(id: Long, name: String) = PosSalesDashboardListItem(
        accountId = id,
        accountName = name,
    ...
    fun emptyExport() {
        ...
        assertThat(result.filename).isEqualTo("pos-sales-2026-05-01-2026-05-31.xlsx")
    ...
    fun nonEmptyExport() {
        ...
        ByteArrayInputStream(result.bytes).use { input ->
            ...
            assertThat(sheet.getRow(0).getCell(4).stringCellValue).isEqualTo("POS매출 금액(원)")
    ...
    fun filenameDateRange() {
        ...
        assertThat(result.filename).isEqualTo("pos-sales-2026-03-05-2026-04-02.xlsx")
    }
}
