...
class OrderRequestProductSFAnnotationTest {
    ...
    inner class LineChangeTypeCancel {
        ...
        // ───────── Spec #858 — 취소 흔적 기록 + 정합 승격 도메인 메서드 ─────────

        @Test
        @DisplayName("markCancelRequested() — cancel_requested 만 세팅, line_change_type 미변경")
        fun markCancelRequestedSetsTraceOnly() {
            val product = newProduct()

            product.markCancelRequested("E001")

            assertThat(product.cancelRequestedBy).isEqualTo("E001")
            assertThat(product.cancelRequestedAt).isNotNull()
            assertThat(product.isCancelled()).isFalse() // 취소 마커는 미변경
        ...
        private fun newProduct(): OrderRequestProduct {
            ...
            val orderRequest = OrderRequest(
                ...
                orderDate = LocalDateTime.of(2026, 5, 14, 10, 0),
                deliveryDate = LocalDate.of(2026, 5, 16),
