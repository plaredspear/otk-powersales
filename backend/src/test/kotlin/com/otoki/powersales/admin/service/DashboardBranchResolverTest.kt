...
class DashboardBranchResolverTest {
    ...
    @Test
    @DisplayName("고정 화이트리스트 - 영업지원2팀(4889)/CVS전략팀(5694) 포함 + 지점명 가나다순(한글 우선) 정렬")
    fun dashboardAllBranches_contents() {
        ...
        assertThat(list.first().branchName).isEqualTo("강남1지점")
        assertThat(list.map { it.branchName }.filter { it.startsWith("강남") })
            .containsExactly("강남1지점", "강남2지점", "강남3지점", "강남4지점")
        // 영문으로 시작하는 이름은 맨 뒤 (한글 우선)
        assertThat(list.last().branchName).isEqualTo("CVS전략팀")
    }
