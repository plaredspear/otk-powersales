...
class AdminPromotionControllerTest : AdminControllerTestSupport() {
    ...
    fun stubArgumentResolver() {
        ...
        // 지점 스코프 산출 — 기본은 전사(Unrestricted, 지점 필터 미적용). branchCode 미지정 케이스 정합.
        // 목록/엑셀(MASTER_LIST) 과 target-actual-report(REPORT) 는 profile 만 다르고 같은 게이트웨이를 탄다.
    ...
    inner class ExportPromotions {
        ...
        fun exportPromotions_success() {
            every {
                adminPromotionService.exportPromotions(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any())
            } returns ExcelResult(
