...
import com.otoki.powersales.external.sap.inbound.controller.SapProductMasterController
...
class SapProductMasterControllerTest {
    ...
    inner class UpsertProduct {
        ...
        @MethodSource("com.otoki.powersales.external.sap.inbound.controller.SapProductMasterControllerTest#invalidProductPayloadCases")
    ...
    inner class UpsertBarcode {
        ...
        @ParameterizedTest(name = "{0} → status={1}, RESULT_CODE=INVALID_PAYLOAD")
        @MethodSource("com.otoki.powersales.external.sap.inbound.controller.SapProductMasterControllerTest#invalidBarcodePayloadCases")
        @DisplayName("실패 - INVALID_PAYLOAD 변형들")
        fun upsert_invalidPayload(case: String, expectedStatus: Int, payload: String) {
            mockMvc.perform(
                post("/api/v1/sap/product-barcode")
    ...
    inner class UpsertSystemCode {
        ...
        @ParameterizedTest(name = "{0} → status={1}, RESULT_CODE=INVALID_PAYLOAD")
        @MethodSource("com.otoki.powersales.external.sap.inbound.controller.SapProductMasterControllerTest#invalidSystemCodePayloadCases")
        @DisplayName("실패 - INVALID_PAYLOAD 변형들")
        fun upsert_invalidPayload(case: String, expectedStatus: Int, payload: String) {
            mockMvc.perform(
                post("/api/v1/sap/system-code")
