...
class LeaderScheduleServiceTest {
    ...
    @Mock
    private lateinit var scheduleConflictValidator: ScheduleConflictValidator

    @InjectMocks
    private lateinit var leaderScheduleService: LeaderScheduleService
    ...
    inner class CreateTeamMemberScheduleTests {
        ...
        fun create_success() {
            // Given
            val leader = createEmployee(id = 4001, authority = "조장", costCenterCode = "C001")
            val target = createEmployee(id = 5012, authority = "여사원", costCenterCode = "C001", status = "활동")
            val account = createAccount(id = 90234, branchCode = "C001", accountGroup = "1000")
            val request = createRequest()

            whenever(employeeRepository.findById(leader.id)).thenReturn(Optional.of(leader))
            whenever(employeeRepository.findById(target.id)).thenReturn(Optional.of(target))
            ...
            verify(scheduleConflictValidator).validateConflicts(
                eq(target.id), eq(LocalDate.parse("2026-05-15")), eq("근무"), eq(account.id), eq("고정")
            )
        }
        ...
        fun create_registrantNotFound() {
            whenever(employeeRepository.findById(4001L)).thenReturn(Optional.empty())
        ...
        fun create_notLeader() {
            val nonLeader = createEmployee(id = 4001, authority = "여사원", costCenterCode = "C001")
            whenever(employeeRepository.findById(nonLeader.id)).thenReturn(Optional.of(nonLeader))
        ...
        fun create_targetNotFound() {
            val leader = createEmployee(id = 4001, authority = "조장", costCenterCode = "C001")
            whenever(employeeRepository.findById(leader.id)).thenReturn(Optional.of(leader))
            whenever(employeeRepository.findById(5012L)).thenReturn(Optional.empty())
        ...
        fun create_notTeamMember() {
            val leader = createEmployee(id = 4001, authority = "조장", costCenterCode = "C001")
            val target = createEmployee(id = 5012, authority = "여사원", costCenterCode = "C002", status = "활동")
            whenever(employeeRepository.findById(leader.id)).thenReturn(Optional.of(leader))
            whenever(employeeRepository.findById(target.id)).thenReturn(Optional.of(target))
        ...
        fun create_targetOnLeave() {
            val leader = createEmployee(id = 4001, authority = "조장", costCenterCode = "C001")
            val target = createEmployee(id = 5012, authority = "여사원", costCenterCode = "C001", status = "휴직")
            whenever(employeeRepository.findById(leader.id)).thenReturn(Optional.of(leader))
            whenever(employeeRepository.findById(target.id)).thenReturn(Optional.of(target))
        ...
        fun create_targetRetired() {
            val leader = createEmployee(id = 4001, authority = "조장", costCenterCode = "C001")
            val target = createEmployee(id = 5012, authority = "여사원", costCenterCode = "C001", status = "퇴직")
            whenever(employeeRepository.findById(leader.id)).thenReturn(Optional.of(leader))
            whenever(employeeRepository.findById(target.id)).thenReturn(Optional.of(target))
        ...
        fun create_accountNotFound() {
            ...
            whenever(employeeRepository.findById(leader.id)).thenReturn(Optional.of(leader))
            whenever(employeeRepository.findById(target.id)).thenReturn(Optional.of(target))
            whenever(accountRepository.findById(any<Int>())).thenReturn(Optional.empty())
    ...
    inner class GetTeamMembersTests {
        ...
        fun getTeamMembers_success() {
            val leader = createEmployee(id = 4001, authority = "조장", costCenterCode = "C001")
            val m1 = createEmployee(id = 5012, employeeCode = "20300002", authority = "여사원", costCenterCode = "C001", status = "활동")
            val m2 = createEmployee(id = 5013, employeeCode = "20300001", authority = "여사원", costCenterCode = "C001", status = "휴직")

            whenever(employeeRepository.findById(leader.id)).thenReturn(Optional.of(leader))
            whenever(employeeRepository.findByCostCenterCodeAndAppAuthority("C001", "여사원"))
