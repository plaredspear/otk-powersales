...
@DisplayName("LastMonthRevenueLookup 테스트 — orora `closingAmountSum` 정합")
class LastMonthRevenueLookupTest {

    private val ororaGateway: OroraMonthlySalesHistoryQueryGateway = mockk(relaxUnitFun = true)
    private val lookup = LastMonthRevenueLookup(ororaGateway)
    ...
    private fun row(sapCode: String, abc1: Long = 0L, ship1: Long = 0L): OroraMonthlySalesHistory =
    ...
    inner class ForAccounts {
        ...
        fun returnsMapWithOnlyMatchedAccountsScaled() {
            ...
            every {
                ororaGateway.findBySalesDate(expectedSalesDate, setOf("S001", "S002", "S003"))
            } returns listOf(
        ...
        fun emptyAccountsReturnsEmptyMapWithoutGatewayCall() {
            ...
            verify(exactly = 0) { ororaGateway.findBySalesDate(any(), any()) }
        }
        ...
        fun filtersOutAccountsWithoutExternalKey() {
            ...
            every { ororaGateway.findBySalesDate(expectedSalesDate, setOf("S001")) } returns
    ...
    inner class ForAccount {
        ...
        fun singleAccountWithRevenueReturnsScaledValue() {
            val a1 = account(1, "S001")
            every { ororaGateway.findBySalesDate(expectedSalesDate, listOf("S001")) } returns
        ...
        fun singleAccountWithoutRevenueReturnsNull() {
            val a1 = account(1, "S001")
            every { ororaGateway.findBySalesDate(expectedSalesDate, listOf("S001")) } returns emptyList()
    ...
    inner class TodayArgument {
        ...
        fun previousMonthRespectsLeftPadConvention() {
            val a1 = account(1, "S001")
            every { ororaGateway.findBySalesDate("202601", listOf("S001")) } returns
                listOf(OroraMonthlySalesHistory(
                    sapAccountCode = "S001",
                    salesDate = "202601",
                    abcClosingAmount1 = BigDecimal("777"),
                ))

            val result = lookup.forAccount(a1, today = LocalDate.of(2026, 2, 15))

            assertThat(result).isEqualTo(BigDecimal("777"))
            verify(exactly = 1) { ororaGateway.findBySalesDate("202601", listOf("S001")) }
        }
    }
}
