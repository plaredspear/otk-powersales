...
import com.otoki.powersales.sap.dto.SapClientOrderRequest
...
import com.otoki.powersales.sap.service.SapClientOrderService
...
@WebMvcTest(SapClientOrderController::class)
@AutoConfigureMockMvc(addFilters = false)
@DisplayName("SapClientOrderController 테스트")
class SapClientOrderControllerTest {
    ...
    @Nested
    @DisplayName("POST /api/v1/sap/client-order")
    inner class SyncClientOrder {

        @Test
        @DisplayName("성공 - 동기화 결과 반환")
        fun `should return SUCCESS when sync succeeds`() {
            // given
            val request = SapClientOrderRequest(
                reqItemList = listOf(
                    SapClientOrderRequest.ReqItem(
                        sapOrderNumber = "ORD001",
                        sapAccountCode = "0001234567",
                        itemDetailList = listOf(
                            SapClientOrderRequest.ItemDetail(sapOrderNumber = "ORD001", lineNumber = "10")
                        )
                    )
                )
            )
            whenever(sapClientOrderService.sync(any()))
                .thenReturn(SapSyncResult(successCount = 1, failCount = 0))

            // when & then
            mockMvc.perform(
                post("/api/v1/sap/client-order")
        ...
        fun `should return EMPTY_REQUEST when reqItemList is empty`() {
            // given
            val request = SapClientOrderRequest(reqItemList = emptyList())
