...
import com.otoki.powersales.sap.dto.SapSystemCodeMasterRequest
...
import com.otoki.powersales.sap.service.SapSystemCodeMasterService
...
@WebMvcTest(SapSystemCodeMasterController::class)
@AutoConfigureMockMvc(addFilters = false)
@DisplayName("SapSystemCodeMasterController 테스트")
class SapSystemCodeMasterControllerTest {
    ...
    @Nested
    @DisplayName("POST /api/v1/sap/system-code-master")
    inner class SyncSystemCodeMaster {

        @Test
        @DisplayName("성공 - 정상 동기화 요청")
        fun sync_success() {
            whenever(sapSystemCodeMasterService.sync(any())).thenReturn(
                SapSyncResult(successCount = 1, failCount = 0)
            )

            val request = SapSystemCodeMasterRequest(
                reqItemList = listOf(
                    SapSystemCodeMasterRequest.ReqItem(
                        companyCode = "1000",
                        groupCode = "H10010",
                        detailCode = "1",
                        groupCodeName = "재직상태",
            ...
            mockMvc.perform(
                post("/api/v1/sap/system-code-master")
        ...
        fun sync_emptyRequest() {
            val request = SapSystemCodeMasterRequest(reqItemList = emptyList())
        ...
        fun sync_transactionFailure() {
            whenever(sapSystemCodeMasterService.sync(any())).thenThrow(
