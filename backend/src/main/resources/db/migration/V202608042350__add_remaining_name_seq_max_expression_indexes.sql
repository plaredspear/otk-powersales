...
--
-- 후속 변경 주의: 같은 PR 후속 커밋에서 MAX 보정을 채번 hot path 에서 떼어내 부팅 1회 /
-- SF 마이그레이션 직후에만 실행하도록 바꿨다 (NameSequenceSyncService — 채번은 nextval 단독).
-- 따라서 위 "루프 증폭" 은 해소됐고, 이 인덱스들은 이제 **보정 시점** 비용을 낮추는 용도다
-- (부팅 시 8개 테이블 전건 스캔 방지).
--
