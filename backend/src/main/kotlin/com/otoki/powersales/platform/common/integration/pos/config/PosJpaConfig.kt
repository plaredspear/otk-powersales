 ...
 *
 * [com.otoki.powersales.platform.common.integration.orora.config.OroraJpaConfig] 와 동일 격리 패턴:
 * - `posDataSource` 위에 별도 `EntityManagerFactory`/`TransactionManager` 등록 (메인 RDS 와 격리).
 * - entity/repository 가 `com.otoki.pos.*` root 로 분리 → 메인
 *   [com.otoki.powersales.platform.common.config.MainJpaRepositoriesConfig] 의 scope 에 미포함.
 *
 * ## read-only 가드
