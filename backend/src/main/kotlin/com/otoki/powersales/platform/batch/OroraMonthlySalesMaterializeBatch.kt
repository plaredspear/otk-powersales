 ...
 * SAP PO 어댑터 callout → ORORA DB 직조회. 레거시는 익월 초 1회 실행되어 **전월** 마감분을 적재했으며
 * (운영자 수동, `OroraYearMonth__mdt` 전월값) 운영 DB 적재 분포도 이를 확인 → 본 batch 는 **월 1회**
 * cron 으로 **전월** 동적 산출. 처리 로직은 [OroraSalesMaterializeFacade.materializeMonthly] 위임.
 *
...
class OroraMonthlySalesMaterializeBatch(
    ...
    @Scheduled(cron = "\${app.batch.orora.monthly.cron:0 0 5 3 * *}")
