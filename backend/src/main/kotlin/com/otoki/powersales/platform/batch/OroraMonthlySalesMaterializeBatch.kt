 ...
 * 신규는 이 수동 관행을 **매일 1회 전월 재적재**로 자동 흡수한다 — `external_key` 멱등 upsert 라
 * 반복 실행이 무해하고, 재마감이 익월 언제 발생하든 다음 발화에서 자동 반영된다.
...
class OroraMonthlySalesMaterializeBatch(
    ...
    // 기본값 매일 11:30 KST (전월 재적재) — 일별 batch (기본 매일 11:00) 와 30분 간격을 두어 ORORA 부하 분산.
    // 레거시 첫째 목요일 1회 + 수동 재실행 관행을 매일 자동 재적재로 대체 (클래스 주석 참조).
    // 실 운영은 환경변수 `app.batch.orora.monthly.cron` override — 본 변경 적용 시 운영값도 매일 발화로 함께 조정 필요.
    // JVM/컨테이너 TZ=Asia/Seoul (Dockerfile) 이므로 zone 명시 없이 KST 로 발화.
    @Scheduled(cron = "\${app.batch.orora.monthly.cron:0 30 11 * * *}")
