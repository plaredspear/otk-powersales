...
class SharingRulePolicyEvaluator(
    ...
    internal fun recordTypeAndPredicate(
        ...
        return try {
            val path = Expressions.numberPath(Long::class.java, entityPath, "recordTypeId")
            if (scope.visibleRecordTypeIds.isEmpty()) {
                ...
                val values: List<Long> = scope.visibleRecordTypeIds.map { it as Long }
    ...
    fun ownerPredicate(scope: DataScope, entityPath: EntityPathBase<*>): BooleanExpression? {
        ...
        return try {
            ...
            val ownerIdPath = Expressions.numberPath(Long::class.java, entityPath, ownerProperty)
            ownerIdPath.eq(userId as Long)
        } catch (e: Exception) {
    ...
    fun hierarchyPredicate(scope: DataScope, entityPath: EntityPathBase<*>): BooleanExpression? {
        ...
        return try {
            val path = Expressions.numberPath(Long::class.java, entityPath, ownerRelation)
            // QueryDSL `.in(Collection<out Long>)` overload 정합 — Set<Long> 을 List<java.lang.Long> 으로 박싱.
            val values: List<Long> = scope.allSubordinateUserRoleIds.map { it as Long }
            path.`in`(values)
        } catch (e: Exception) {
    ...
    private fun buildAuditOwnerConditionPredicate(
        ...
        return try {
            @Suppress("UNCHECKED_CAST")
            val path = Expressions.numberPath(Long::class.java, entityPath, relationPath)
            when (cond.operator) {
                "equals" -> path.eq(resolvedUserId as Long)
                "notEqual" -> path.ne(resolvedUserId as Long)
                else -> {
                    log.warn(
