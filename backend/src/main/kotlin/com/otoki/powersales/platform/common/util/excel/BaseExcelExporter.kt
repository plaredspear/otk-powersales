 ...
 *
 * 서브클래스는 [headers] / [sheetName] / [writeRow] 3가지만 구현하면
 * 워크북 생성 / 헤더 스타일 / freeze pane / autoSize / 바이트 직렬화 / [ExcelResult] 래핑을 자동 처리한다.
 *
 * 파일명은 두 가지 방식 중 택일:
 *  - 정적: [defaultFilename] 을 override 하고 `export(items)` 호출
 *  - 동적(날짜/기간 등 인자 기반): `export(items, filename)` 오버로드에 파일명을 전달
 *
...
abstract class BaseExcelExporter<T> {
    ...
    /** 정적 파일명 기본값. `export(items)` 호출 시 사용. 동적 파일명은 `export(items, filename)` 사용. */
    protected open val defaultFilename: String = "export.xlsx"
    ...
    /** 정적 파일명([defaultFilename]) 으로 export. */
    fun export(items: List<T>): ExcelResult = export(items, defaultFilename)

    /** 동적 파일명으로 export — 날짜/기간 등 인자 기반 파일명이 필요한 호출처가 사용. */
    fun export(items: List<T>, filename: String): ExcelResult {
