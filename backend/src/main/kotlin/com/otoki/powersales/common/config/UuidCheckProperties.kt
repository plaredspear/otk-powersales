 ...
 * - `excludedEmployeeNumbers` 에 포함된 사번은 단말 바인딩 검증을 면제한다 (CSV).
 */
@ConfigurationProperties(prefix = "app.auth.uuid-check")
data class UuidCheckProperties(
    val enabled: Boolean = true,
    val excludedEmployeeNumbers: String = ""
) {
    fun isExcluded(employeeCode: String): Boolean {
        if (excludedEmployeeNumbers.isBlank()) return false
        return excludedEmployeeNumbers.split(",").map { it.trim() }.contains(employeeCode)
    }
}
