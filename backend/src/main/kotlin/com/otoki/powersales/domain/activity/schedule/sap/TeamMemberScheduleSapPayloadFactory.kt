...
/**
 * 여사원일정 근무(REGULAR) SAP 페이로드 빌더 (Spec #588 P1-B §2.2).
 *
 ...
 * - wrapper `{ "request": [ ... ] }` 는 [TeamMemberScheduleSapPayload] 생성 책임
 */
@Component
class TeamMemberScheduleSapPayloadFactory {

    fun build(rows: List<TeamMemberScheduleSapPayloadRow>, today: LocalDate): TeamMemberScheduleSapPayload {
        return TeamMemberScheduleSapPayload(
            request = rows.map { toItem(it, today) }
        )
    }

    private fun toItem(row: TeamMemberScheduleSapPayloadRow, today: LocalDate): TeamMemberScheduleSapItem {
        val isYesterdayCorrection = row.workingDate.isBefore(today)
        return TeamMemberScheduleSapItem(
...
data class TeamMemberScheduleSapPayload(
    val request: List<TeamMemberScheduleSapItem>
)

/** SAP 단일 element. 키 명명은 레거시 SF 그대로 (PascalCase). */
@Suppress("ConstructorParameterNaming", "PropertyName")
data class TeamMemberScheduleSapItem(
