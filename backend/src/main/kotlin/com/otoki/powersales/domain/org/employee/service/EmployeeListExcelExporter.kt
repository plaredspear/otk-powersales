...
class EmployeeListExcelExporter : BaseExcelExporter<EmployeeListItem>() {
    ...
    // 목록 테이블 컬럼 순서 정합 (사번 ~ 앱활성). 화면과 동일하게 성별·지점코드 미포함, 전문행사조·근무형태·근무거래처 포함.
    override val headers = listOf(
        "사번", "이름", "상태", "소속", "전문행사조", "근무형태", "근무거래처", "거래처코드", "권한",
    ...
    override fun writeRow(row: Row, item: EmployeeListItem) {
        ...
        row.createCell(5).setCellValue(item.workType ?: "-")
        row.createCell(6).setCellValue(item.workAccountName ?: "-")
        row.createCell(7).setCellValue(item.workAccountCode ?: "-")
        row.createCell(8).setCellValue(item.role ?: "")
        row.createCell(9).setCellValue(item.jikjong ?: "")
        row.createCell(10).setCellValue(item.jikchak ?: "")
        row.createCell(11).setCellValue(item.jikwee ?: "")
        row.createCell(12).setCellValue(item.jikgub ?: "")
        row.createCell(13).setCellValue(item.workEmail ?: "")
        row.createCell(14).setCellValue(item.phone ?: "")
        row.createCell(15).setCellValue(item.appointmentDate ?: "")
        row.createCell(16).setCellValue(item.ordDetailNode ?: "")
        row.createCell(17).setCellValue(item.startDate ?: "")
        // 재직 중인 사원은 퇴사일을 표시하지 않는다 (화면 정합).
        row.createCell(18).setCellValue(if (item.status == "재직") "" else (item.endDate ?: ""))
        row.createCell(19).setCellValue(item.age ?: "")
        row.createCell(20).setCellValue(item.yearsOfService ?: "")
        row.createCell(21).setCellValue(if (item.appLoginActive == true) "활성" else "비활성")
    }
}
