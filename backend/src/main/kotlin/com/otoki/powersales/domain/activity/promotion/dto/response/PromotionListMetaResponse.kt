...
/**
 * 행사마스터 목록 화면 조회 조건 로드 응답 — "권한 기반 조건/UI 제어" 표준 패턴 파일럿.
 *
 * 목록 진입 시 화면이 필요로 하는 조회 조건(필터 옵션)과 기본값을 한 번에 내려준다.
 * 기존 `/branches`(지점 셀렉터) + `/form-meta`(행사유형)로 분산되어 있던 조건 로드를 단일 응답으로 통합한다.
 *
 ...
 */
data class PromotionListMetaResponse(
    val filters: List<PromotionFilterMeta>,
    val defaults: PromotionListDefaults,
)

/** 조회 조건 1개의 메타. SELECT 계열만 [options] 를 채우고, TEXT/DATE 는 null. */
data class PromotionFilterMeta(
    val key: String,
    val type: PromotionFilterType,
    val options: List<PromotionFilterOption>? = null,
)

enum class PromotionFilterType {
    TEXT,
    SELECT,
    DATE,
}

data class PromotionFilterOption(
    val value: String,
    val label: String,
)

/** 목록 최초 조회 기본값. */
data class PromotionListDefaults(
    val pageSize: Int,
    val sort: String,
)
