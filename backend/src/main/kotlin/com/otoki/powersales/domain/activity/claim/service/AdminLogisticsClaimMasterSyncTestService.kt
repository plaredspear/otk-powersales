...
/**
 * SF Apex REST `/IF_SendLogisticsClaimToPWS` 물류 클레임 마스터 조회 테스트 서비스 (개발자 도구 — 외부 API 테스트).
 *
 * `MOD_DT`(기준 일자) 하나를 SF 로 POST 하면 SF 가 해당 일자 기준으로 변경된 물류 클레임(제안) 마스터 목록을
 * 응답하는 SF → PWS 조회 인터페이스. 신규 DB 에는 저장하지 않고 SF 응답 원형만 그대로 반환한다
 * (일반 클레임 마스터 조회([AdminClaimMasterSyncTestService]) 와 동일 패턴, endpoint 만 다르다).
 *
 * 처리:
...
class AdminLogisticsClaimMasterSyncTestService(
    ...
    fun test(
        ...
        val response = try {
            ...
            log.warn("[logistics-claim-master-sync-test] SF OAuth 실패: {}", e.message)
            AdminLogisticsClaimMasterSyncTestResponse(
                success = false,
                resultCode = null,
            ...
            log.warn("[logistics-claim-master-sync-test] SF 호출 예외: {}", e.message)
            AdminLogisticsClaimMasterSyncTestResponse(
                success = false,
                resultCode = null,
        ...
        log.info(
            "SF_LOGISTICS_CLAIM_MASTER_SYNC_TEST user={} modDt={} success={} resultCode={}",
