 ...
 * 주문요청 상태 enum
 *
 * 라이프사이클: DRAFT → SENT → APPROVED 또는 SEND_FAILED → (사용자 전량취소 시) CANCEL_REQUESTED
 *
...
enum class OrderRequestStatus(
     ...
     *  - `CANCEL_REQUESTED`("주문취소") → "주문취소요청완료" (실제 취소는 SAP 접수 후 시간이 걸릴 수 있음)
     *
    ...
    val clientDisplayName: String
        get() = when (this) {
            APPROVED -> "전송완료"
            CANCEL_REQUESTED -> "주문취소요청완료"
            else -> displayName
        }

    companion object {
        /** 취소 가능 상태 (Spec #597). `DRAFT` / `CANCEL_REQUESTED` 는 취소 불가. */
