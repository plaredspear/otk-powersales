 ...
 *
 * @property pwrskey                신규 제안의 primary key(`suggestion_id`) — 신규 시스템 생성분을 SF 가 echo.
 *                                  존재하면 이 값(suggestion_id)으로 suggestion 을 우선 조회해 매칭한다.
 * @property name                   SF 표준 Name(제안번호, `proposal_number`) — SF 에서 생성한 제안의 자연키.
 *                                  pwrskey 가 없을 때(SF 단독 생성분) 이 값으로 suggestion 을 조회해 매칭한다.
...
data class LogisticsClaimMasterSfRecord(
    ...
    /** pwrskey 를 suggestion PK(Long) 로 파싱. 비어있거나 숫자가 아니면 null (PK 매칭 불가 — name fallback 대상). */
    fun pwrskeyAsSuggestionId(): Long? = pwrskey?.takeIf { it.isNotBlank() }?.trim()?.toLongOrNull()

    /** name(제안번호) 을 매칭 키로 정규화. 비어있으면 null (매칭 불가). */
    fun nameAsProposalNumber(): String? = name?.trim()?.takeIf { it.isNotBlank() }
