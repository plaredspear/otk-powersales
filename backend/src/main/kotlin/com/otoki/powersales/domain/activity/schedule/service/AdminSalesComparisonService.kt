...
import com.otoki.powersales.domain.activity.schedule.dto.response.SalesComparisonDetailItem
import com.otoki.powersales.domain.activity.schedule.dto.response.SalesComparisonDetailResponse
import com.otoki.powersales.domain.activity.schedule.dto.response.SalesComparisonDetailTotal
import com.otoki.powersales.domain.activity.schedule.dto.response.SalesComparisonMiddleItem
import com.otoki.powersales.domain.activity.schedule.dto.response.SalesComparisonMiddleResponse
import com.otoki.powersales.domain.activity.schedule.dto.response.SalesComparisonMiddleSubtotal
import com.otoki.powersales.domain.activity.schedule.dto.response.SalesComparisonSummaryResponse
import com.otoki.powersales.domain.activity.schedule.dto.response.SalesComparisonSummaryRow
...
class AdminSalesComparisonService(
    ...
    fun getDetail(
        ...
        val items = accountSuitabilities
            ...
            .map { (suit, item) -> suit to suit.toDetailItem(item) }
            // SF `lowDataItems.compareTo` 정합 — 거래처유형코드 → 거래처명 → 적합성 → 사원명.
            // 거래처유형코드는 화면 미표시 (응답 DTO 에 없음) 라 산출 모델 쪽 값으로 정렬한다.
            .sortedWith(
                compareBy(
                    { (suit, _) -> suit.accountCategoryCode },
                    { (_, dto) -> dto.accountName },
                    { (_, dto) -> suitabilityOrder(dto.suitability) },
                    { (_, dto) -> dto.employeeName }
                )
            )
            .map { (_, dto) -> dto }
    ...
    private fun AccountSuitability.toDetailItem(
        item: MonthlyIntegrationScheduleItem
    ...
    private fun aggregateMiddle(label: String, list: List<AccountSuitability>): SalesComparisonMiddleSubtotal {
        return SalesComparisonMiddleSubtotal(
            ...
            totalDisplayConvertedHeadcount = list.fold(BigDecimal.ZERO) { acc, it -> acc.add(it.totalDisplayConvertedHeadcount) }
                .setScale(3, RoundingMode.HALF_UP),
            totalEventConvertedHeadcount = list.fold(BigDecimal.ZERO) { acc, it -> acc.add(it.totalEventConvertedHeadcount) }
                .setScale(3, RoundingMode.HALF_UP),
            totalInputCount = list.sumOf { it.totalInputCount },
            totalEquivalentWorkingDays = list.fold(BigDecimal.ZERO) { acc, it -> acc.add(it.totalEquivalentWorkingDays) }
                .setScale(3, RoundingMode.HALF_UP),
    ...
    private fun buildDetailTotal(items: List<SalesComparisonDetailItem>): SalesComparisonDetailTotal {
        return SalesComparisonDetailTotal(
            ...
            totalDisplayConvertedHeadcount = items.fold(BigDecimal.ZERO) { acc, it -> acc.add(it.totalDisplayConvertedHeadcount) }
                .setScale(3, RoundingMode.HALF_UP),
            totalEventConvertedHeadcount = items.fold(BigDecimal.ZERO) { acc, it -> acc.add(it.totalEventConvertedHeadcount) }
                .setScale(3, RoundingMode.HALF_UP),
            totalInputCount = items.sumOf { it.inputCount },
            totalEquivalentWorkingDays = items.fold(BigDecimal.ZERO) { acc, it -> acc.add(it.equivalentWorkingDays) }
                .setScale(3, RoundingMode.HALF_UP),
            totalConvertedHeadcount = items.fold(BigDecimal.ZERO) { acc, it -> acc.add(it.convertedHeadcount) }
                .setScale(3, RoundingMode.HALF_UP),
