...
interface ProductRepositoryCustom {
     ...
     *
     * [recentlyOrderedProductIds] 에 담긴 제품을 결과 상단으로 올리고
     * [ProductSearchRow.recentlyOrdered] 에 표시한다. 비어 있으면 기존 정렬만 적용한다.
     *
     * 목록은 호출자가 별도 조회해서 넘긴다(상관 서브쿼리로 매 행 평가하지 않기 위함) —
     * 정렬용 IN 절 파라미터가 되므로 호출자가 크기를 제한해야 한다.
     */
    fun searchForOrder(
        query: String,
        category2: String?,
        category3: String?,
        pageable: Pageable,
        recentlyOrderedProductIds: Collection<Long> = emptyList()
