...
/**
 * 주문 등록/재전송 임시 차단 가드 (운영(prod) + 개발(dev) 서버 대상).
 *
 * 활성 프로파일에 `prod` 또는 `dev` 가 포함되면 주문 등록([OrderRequestCreateService.create]) 및
 * 재전송([OrderRequestResendService.resend]) 진입부에서 [OrderRegistrationBlockedException] 을 던져
 * SAP 호출·DB 적재 이전에 전면 차단한다. local 은 영향 없이 정상 동작한다.
 *
...
class OrderRegistrationBlockGuard(environment: Environment) {
    ...
    /** prod / dev 환경이면 즉시 [OrderRegistrationBlockedException] 을 던진다. */
    ...
    companion object {
        private val BLOCKED_PROFILES = setOf("prod", "dev")
    }
}
