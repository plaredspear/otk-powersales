...
class AdminClaimMasterSyncTestService(
    ...
    fun test(
        ...
        val response = try {
            ...
            log.warn("[claim-master-sync-test] SF OAuth 실패: {}", e.message)
            AdminClaimMasterSyncTestResponse(
                success = false,
                resultCode = null,
            ...
            log.warn("[claim-master-sync-test] SF 호출 예외: {}", e.message)
            AdminClaimMasterSyncTestResponse(
                success = false,
                resultCode = null,
        ...
        log.info(
            "SF_CLAIM_MASTER_SYNC_TEST user={} modDt={} success={} resultCode={}",
