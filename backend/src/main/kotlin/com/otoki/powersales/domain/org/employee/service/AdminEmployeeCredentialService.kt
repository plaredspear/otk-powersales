 ...
 * - 비밀번호 임시 리셋: 임시 비밀번호 `"{사번}@pwrs"` ([TemporaryPasswordPolicy]) 를 BCrypt 해시화하여
 *   저장 + `passwordChangeRequired = true`
 *
...
class AdminEmployeeCredentialService(
    ...
    fun resetPassword(employeeId: Long): ResetPasswordResponse {
        val employee = findActiveEmployee(employeeId)
        val encoded = passwordEncoder.encode(TemporaryPasswordPolicy.forEmployeeCode(employee.employeeCode))!!
