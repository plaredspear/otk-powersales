...
class ElectronicSalesAdminQueryService(
    ...
    private fun aggregateCustomers(
        ...
        if (barcodes.isEmpty()) {
            return liveTotSalesDailyRepository.aggregateByCustomer(custCds, from, to)
                .associate { row ->
                    row.getCustCd() to (
        ...
        barcodes.chunked(BARCODE_CHUNK_SIZE).forEach { chunk ->
            liveTotSalesDailyRepository.aggregateByCustomerAndBarcodes(custCds, from, to, chunk)
                .forEach { row ->
