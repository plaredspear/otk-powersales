...
class RdpMfeisQueryService(
    ...
    fun search(request: MfeisSearchRequest, clientId: String?, clientIp: String): MfeisPageResponse {
        val year = request.year?.takeUnless { it.isBlank() }
            ?: throw RdpInvalidParameterException("year 는 필수입니다")
        val month = request.month?.takeUnless { it.isBlank() }
            ?: throw RdpInvalidParameterException("month 는 필수입니다")
    ...
    companion object {
        const val ENDPOINT = "/api/v1/rdp/mfeis/search"
