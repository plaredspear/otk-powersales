...
import com.otoki.powersales.external.sap.inbound.dto.attendance.AttendInfoDetail
...
class SapAttendInfoService(
    ...
    fun insert(items: List<AttendInfoRequestItem>): AttendInfoDetail {
        ...
        var totalSuccess = 0
        var aggregatedSummary = ScheduleConversionSummary.Companion.ZERO
        ...
        chunks.forEachIndexed { idx, chunk ->
            ...
            try {
                ...
                val status = when {
                    result.failures.isEmpty() -> ChunkResult.Companion.STATUS_SUCCESS
                    result.successCount == 0 -> ChunkResult.Companion.STATUS_FAILED
                    else -> ChunkResult.Companion.STATUS_PARTIAL
                }
                ...
                chunkResults += ChunkResult(idx, ChunkResult.Companion.STATUS_FAILED, chunk.size, reason)
                chunk.forEach { item ->
