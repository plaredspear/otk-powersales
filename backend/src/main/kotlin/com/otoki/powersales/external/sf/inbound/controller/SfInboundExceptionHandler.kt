...
@RestControllerAdvice(
    assignableTypes = [
        SfTokenController::class,
        SfInboundHealthController::class
    ]
)
...
class SfInboundExceptionHandler {
    ...
    fun handleValidation(ex: MethodArgumentNotValidException): ResponseEntity<SfResultWrapper<Nothing>> {
        ...
        return json(HttpStatus.BAD_REQUEST, SfResultWrapper(SfResultWrapper.Companion.CODE_INVALID_PAYLOAD, msg))
    }

    @ExceptionHandler(HttpMessageNotReadableException::class)
    fun handleUnreadable(): ResponseEntity<SfResultWrapper<Nothing>> {
        return json(HttpStatus.BAD_REQUEST,
            SfResultWrapper(SfResultWrapper.Companion.CODE_INVALID_PAYLOAD, "요청 본문이 올바르지 않습니다")
        )
    }
    ...
    fun handleAccessDenied(ex: AccessDeniedException): ResponseEntity<SfResultWrapper<Nothing>> {
        return json(HttpStatus.FORBIDDEN, SfResultWrapper(SfResultWrapper.Companion.CODE_INSUFFICIENT_SCOPE, "권한 없음"))
    }

    @ExceptionHandler(Exception::class)
    fun handleAny(ex: Exception): ResponseEntity<SfResultWrapper<Nothing>> {
        return json(HttpStatus.INTERNAL_SERVER_ERROR,
            SfResultWrapper(SfResultWrapper.Companion.CODE_INTERNAL_ERROR, "내부 오류")
        )
    }
}
