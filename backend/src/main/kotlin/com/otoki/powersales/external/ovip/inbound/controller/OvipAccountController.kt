...
import com.otoki.powersales.external.ovip.inbound.service.OvipAccountQueryService
...
/**
 * OVIP 인바운드 — 거래처(Account) 전량 스냅샷 조회 API.
 *
 * 부하 방지 계약:
 * - PK keyset 커서(`cursor`) → offset 저하 없이 대량 순차 인출
 * - `size` 상한 clamp (ovip.inbound.account.max-page-size)
 *
 ...
 */
@RestController
@RequestMapping("/api/v1/ovip/account")
class OvipAccountController(
    private val queryService: OvipAccountQueryService
) {
    ...
    ): ResponseEntity<OvipResultWrapper<SnapshotPageResponse<AccountRow>>> {
