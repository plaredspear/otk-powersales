 ...
 * @property employee 사원(Employee) 전량 스냅샷 조회 페이지네이션 설정
 */
...
data class RdpInboundProperties(
    ...
    val employee: Pagination = Pagination()
) {
    /**
    ...
    /**
     * 마스터 전량 스냅샷 페이지네이션 설정 (거래처 / 사원 공용).
     *
     * MFEIS 대비 상한이 낮은 이유: 두 마스터는 entity 전 컬럼을 노출하므로 row 당 페이로드가
