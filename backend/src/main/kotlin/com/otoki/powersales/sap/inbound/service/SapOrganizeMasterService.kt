 ...
 * - `@SapInboundAccepted` annotation — `REQUEST_ACCEPTED` audit (reason="success={N} failure=0") 은 [com.otoki.powersales.sap.auth.audit.SapInboundAuditAspect]
 *   가 처리 (#639). sanity aspect 가 별도로 기록하는 reason 미포함 `REQUEST_ACCEPTED` 와 공존하며,
 *   sanity 거부 시 Aspect 는 [com.otoki.powersales.sap.auth.exception.SapSanityCheckFailedException] 을 catch 하여 즉시 rethrow (audit 미기록).
 *
