...
class SapInboundExceptionHandler(
    ...
    fun handleValidation(ex: MethodArgumentNotValidException): ResponseEntity<SapResultWrapper<Nothing>> {
        ...
        return json(HttpStatus.BAD_REQUEST, SapResultWrapper(SapResultWrapper.CODE_INVALID_PAYLOAD, msg))
    }

    @ExceptionHandler(HttpMessageNotReadableException::class)
    fun handleUnreadable(): ResponseEntity<SapResultWrapper<Nothing>> {
        return json(HttpStatus.BAD_REQUEST, SapResultWrapper(SapResultWrapper.CODE_INVALID_PAYLOAD, "요청 본문이 올바르지 않습니다"))
    }
    ...
    fun handleMediaTypeNotSupported(ex: HttpMediaTypeNotSupportedException): ResponseEntity<SapResultWrapper<Nothing>> {
        ...
        return json(
            ...
            SapResultWrapper(SapResultWrapper.CODE_UNSUPPORTED_MEDIA_TYPE, "지원하지 않는 Content-Type. 허용: $supported")
        )
    }
    ...
    fun handleNotAcceptable(): ResponseEntity<SapResultWrapper<Nothing>> {
        return json(
            ...
            SapResultWrapper(SapResultWrapper.CODE_NOT_ACCEPTABLE, "Accept 헤더가 application/json 을 포함해야 합니다")
        )
    }
    ...
    fun handleAccessDenied(ex: AccessDeniedException): ResponseEntity<SapResultWrapper<Nothing>> {
        ...
        return json(HttpStatus.FORBIDDEN, SapResultWrapper(SapResultWrapper.CODE_INSUFFICIENT_SCOPE, "권한 없음"))
    }

    @ExceptionHandler(Exception::class)
    fun handleAny(ex: Exception): ResponseEntity<SapResultWrapper<Nothing>> {
        return json(HttpStatus.INTERNAL_SERVER_ERROR, SapResultWrapper(SapResultWrapper.CODE_INTERNAL_ERROR, "내부 오류"))
    }
