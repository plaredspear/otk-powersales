...
class SapAttendInfoService(
    ...
    private fun processAnnualLeaveSchedule(attendInfo: AttendInfo) {
        ...
        if (attendInfo.status == "Y") {
            ...
            createAnnualLeaveSchedules(employee, employeeCode, dates)
        }
    }

    private fun createAnnualLeaveSchedules(employee: com.otoki.powersales.sap.entity.Employee, employeeCode: String, dates: List<LocalDate>) {
        for (date in dates) {
            val exists = teamMemberScheduleRepository.existsByEmployeeAndWorkingDateAndWorkingType(
                employee, date, WORKING_TYPE_ANNUAL_LEAVE
            )
            if (exists) {
