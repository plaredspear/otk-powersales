...
import com.otoki.powersales.sap.dto.SapSyncResponse
...
@RestController
@RequestMapping("/api/v1/sap/system-code-master")
class SapSystemCodeMasterController(
    private val sapSystemCodeMasterService: SapSystemCodeMasterService,
    private val sapSyncLogService: SapSyncLogService
) {

    @PostMapping
    fun syncSystemCodeMaster(
        @RequestBody request: SapSystemCodeMasterRequest,
        ...
        return try {
            val result = sapSystemCodeMasterService.sync(request.reqItemList)
            val durationMs = System.currentTimeMillis() - startTime

            sapSyncLogService.log(
                apiName = "system-code-master",
