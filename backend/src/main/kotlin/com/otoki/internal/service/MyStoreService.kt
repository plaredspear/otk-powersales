...
class MyStoreService(
     ...
     * 2. Account 마스터에서 대표자명, 전화번호 등 추가 정보 병합
    ...
    fun getMyStores(userId: Long, keyword: String?): MyStoreListResponse {
        ...
        // 2. Account 마스터에서 추가 정보 조회
        val accountMap = accountRepository.findByIdIn(distinctStoreIds)
            .associateBy { it.id }
        ...
        val storeInfoList = distinctStoreIds.mapNotNull { storeId ->
            buildMyStoreInfo(storeId, accountMap, scheduleMap)
        }
    ...
    private fun buildMyStoreInfo(
        ...
        val account = accountMap[storeId]
