...
void main() {
  group('FavoriteProductProvider', () {
    ...
    test('즐겨찾기를 추가할 수 있다', () async {
      ...
      await notifier.addFavorite('prod001', '진라면');
    ...
    test('즐겨찾기를 제거할 수 있다', () async {
      ...
      await notifier.removeFavorite('prod001');
    ...
    test('즐겨찾기를 토글할 수 있다 (추가)', () async {
      ...
      await notifier.toggleFavorite('prod001', '진라면');
    ...
    test('즐겨찾기를 검색할 수 있다', () async {
      ...
      await notifier.addFavorite('prod001', '진라면');
      await notifier.addFavorite('prod002', '진라면 매운맛');
      await notifier.addFavorite('prod003', '케첩');
    ...
    test('모든 즐겨찾기를 삭제할 수 있다', () async {
      ...
      await notifier.addFavorite('prod001', '진라면');
      await notifier.addFavorite('prod002', '케첩');
    ...
    test('isFavorite 메서드가 올바르게 동작한다', () async {
      ...
      expect(await notifier.isFavorite('prod001'), true);
      expect(await notifier.isFavorite('prod999'), false);
    });
    ...
    test('여러 즐겨찾기를 추가하면 최신순으로 정렬된다', () async {
      ...
      await notifier.addFavorite('prod002', '케첩');
      await Future.delayed(const Duration(milliseconds: 10));
      await notifier.addFavorite('prod003', '카레');

      final state = container.read(favoriteProductsProvider);
      final favorites = state.value!;

      // 최신순 확인 (가장 마지막에 추가된 것이 첫 번째)
      expect(favorites[0].id, 'prod003');
      expect(favorites[1].id, 'prod002');
      expect(favorites[2].id, 'prod001');
    });
    ...
    test('isFavoriteProvider가 올바르게 동작한다', () async {
      ...
      final isFavorite1 = await container.read(isFavoriteProvider('prod001').future);
      final isFavorite2 = await container.read(isFavoriteProvider('prod999').future);
