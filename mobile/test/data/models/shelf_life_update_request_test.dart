import 'package:flutter_test/flutter_test.dart';
import 'package:mobile/data/models/shelf_life_update_request.dart';
import 'package:mobile/domain/entities/shelf_life_form.dart';

void main() {
  group('ShelfLifeUpdateRequest', () {
    const testRequest = ShelfLifeUpdateRequest(
      expiryDate: '2026-04-01',
      alertDate: '2026-03-25',
      description: '수정된 메모',
    );

    group('toJson', () {
      test('snake_case JSON 키로 올바르게 직렬화해야 한다', () {
        final result = testRequest.toJson();

        expect(result['expiry_date'], '2026-04-01');
        expect(result['alert_date'], '2026-03-25');
        expect(result['description'], '수정된 메모');
      });

      test('description이 빈 문자열일 때도 직렬화해야 한다', () {
        const request = ShelfLifeUpdateRequest(
          expiryDate: '2026-04-01',
          alertDate: '2026-03-25',
        );

        final result = request.toJson();

        expect(result['description'], '');
      });
    });

    group('fromForm', () {
      test('ShelfLifeUpdateForm에서 올바르게 생성해야 한다', () {
        final form = ShelfLifeUpdateForm(
          expiryDate: DateTime(2026, 4, 1),
          alertDate: DateTime(2026, 3, 25),
          description: '수정된 메모',
        );

        final result = ShelfLifeUpdateRequest.fromForm(form);

        expect(result.expiryDate, '2026-04-01');
        expect(result.alertDate, '2026-03-25');
        expect(result.description, '수정된 메모');
      });

      test('DateTime을 YYYY-MM-DD 형식으로 변환해야 한다', () {
        final form = ShelfLifeUpdateForm(
          expiryDate: DateTime(2026, 1, 5),
          alertDate: DateTime(2025, 12, 29),
        );

        final result = ShelfLifeUpdateRequest.fromForm(form);

        expect(result.expiryDate, '2026-01-05');
        expect(result.alertDate, '2025-12-29');
      });

      test('description이 빈 문자열인 Form도 처리해야 한다', () {
        final form = ShelfLifeUpdateForm(
          expiryDate: DateTime(2026, 4, 1),
          alertDate: DateTime(2026, 3, 25),
        );

        final result = ShelfLifeUpdateRequest.fromForm(form);

        expect(result.description, '');
      });
    });

    group('round trip', () {
      test('fromForm -> toJson 변환이 올바른 JSON을 생성해야 한다', () {
        final form = ShelfLifeUpdateForm(
          expiryDate: DateTime(2026, 4, 1),
          alertDate: DateTime(2026, 3, 25),
          description: '수정된 메모',
        );

        final request = ShelfLifeUpdateRequest.fromForm(form);
        final json = request.toJson();

        expect(json, {
          'expiry_date': '2026-04-01',
          'alert_date': '2026-03-25',
          'description': '수정된 메모',
        });
      });
    });

    group('equality', () {
      test('같은 값을 가진 두 요청은 동일해야 한다', () {
        const request1 = ShelfLifeUpdateRequest(
          expiryDate: '2026-04-01',
          alertDate: '2026-03-25',
          description: '수정된 메모',
        );
        const request2 = ShelfLifeUpdateRequest(
          expiryDate: '2026-04-01',
          alertDate: '2026-03-25',
          description: '수정된 메모',
        );

        expect(request1, request2);
      });

      test('다른 값을 가진 두 요청은 동일하지 않아야 한다', () {
        const request2 = ShelfLifeUpdateRequest(
          expiryDate: '2026-05-01',
          alertDate: '2026-04-24',
          description: '다른 메모',
        );

        expect(testRequest, isNot(request2));
      });
    });

    group('hashCode', () {
      test('같은 값을 가진 두 요청은 같은 hashCode를 가져야 한다', () {
        const request1 = ShelfLifeUpdateRequest(
          expiryDate: '2026-04-01',
          alertDate: '2026-03-25',
          description: '수정된 메모',
        );
        const request2 = ShelfLifeUpdateRequest(
          expiryDate: '2026-04-01',
          alertDate: '2026-03-25',
          description: '수정된 메모',
        );

        expect(request1.hashCode, request2.hashCode);
      });
    });

    group('toString', () {
      test('올바른 문자열 표현을 반환해야 한다', () {
        final result = testRequest.toString();

        expect(result, contains('ShelfLifeUpdateRequest'));
        expect(result, contains('expiryDate: 2026-04-01'));
        expect(result, contains('alertDate: 2026-03-25'));
        expect(result, contains('description: 수정된 메모'));
      });
    });

    group('기본값', () {
      test('description 기본값은 빈 문자열이어야 한다', () {
        const request = ShelfLifeUpdateRequest(
          expiryDate: '2026-04-01',
          alertDate: '2026-03-25',
        );

        expect(request.description, '');
      });
    });
  });
}
