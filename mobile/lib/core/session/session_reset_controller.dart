...
enum LogoutReason {
  ...
  /// 자동로그인 미설정 사용자가 장시간 백그라운드 체류 후 복귀해 자동 로그아웃됨.
  /// (레거시 세션 비활동 만료 30분 정합)
  inactivityTimeout,

  /// 서버 데이터 정비(마이그레이션)로 발급시각 컷오프 이전 토큰이 일괄 무효화됨
  /// (서버 `SESSION_INVALIDATED`). 갱신으로 회복되지 않으므로 재로그인만이 정답이다.
  sessionInvalidated,
}
