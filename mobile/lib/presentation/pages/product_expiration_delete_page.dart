...
class ProductExpirationDeletePage extends ConsumerStatefulWidget {
  ...
  const ProductExpirationDeletePage({super.key, required this.items});

  @override
  ConsumerState<ProductExpirationDeletePage> createState() =>
      _ProductExpirationDeletePageState();
}

class _ProductExpirationDeletePageState extends ConsumerState<ProductExpirationDeletePage> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(productExpirationDeleteProvider.notifier).setItems(widget.items);
    });
  }

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(productExpirationDeleteProvider);

    // 삭제 완료 리스닝
    ref.listen(productExpirationDeleteProvider, (previous, next) {
      ...
      if (next.errorMessage != null && previous?.errorMessage == null) {
        ...
        ref.read(productExpirationDeleteProvider.notifier).clearError();
      }
    });
  ...
  Widget _buildSelectAllHeader(ProductExpirationDeleteState state) {
    return InkWell(
      onTap: () {
        ref.read(productExpirationDeleteProvider.notifier).toggleAll();
      },
      child: Padding(
    ...
    return ListView(
      children: [
        ...
        if (state.expiredItems.isNotEmpty) ...[
          ...
          ...state.expiredItems.map((item) => ProductExpirationDeleteCard(
                item: item,
                isSelected: state.selectedSeqs.contains(item.seq),
                onChanged: (_) {
                  ref.read(productExpirationDeleteProvider.notifier).toggleItem(item.seq);
                },
              )),
        ...
        if (state.activeItems.isNotEmpty) ...[
          ...
          ...state.activeItems.map((item) => ProductExpirationDeleteCard(
                item: item,
                isSelected: state.selectedSeqs.contains(item.seq),
                onChanged: (_) {
                  ref.read(productExpirationDeleteProvider.notifier).toggleItem(item.seq);
  ...
  Widget _buildGroupCheckbox({
    ...
    return InkWell(
      onTap: () {
        ref
            .read(productExpirationDeleteProvider.notifier)
  ...
  Widget _buildDeleteButton(ProductExpirationDeleteState state) {
    return SafeArea(
      child: Padding(
