...
/// 소비기한 목록 조회 UseCase
///
/// 검색 필터 조건에 따라 소비기한 목록을 조회합니다.
/// 날짜 범위 유효성을 검증하고, 결과를 그룹화하여 반환할 수 있습니다.
class GetProductExpirationList {
  final ProductExpirationRepository _repository;

  GetProductExpirationList(this._repository);
  ...
  Future<List<ProductExpirationItem>> call(ProductExpirationFilter filter) async {
    if (!filter.isValidDateRange) {
      throw Exception('소비기한 검색 기간은 최대 6개월입니다');
    }

    return await _repository.getProductExpirationList(filter);
  }

  /// 소비기한 목록을 그룹화하여 반환
  ///
  /// "소비기한 지남" (isExpired == true)과 "소비기한 전" (isExpired == false)으로 분리합니다.
  /// 각 그룹 내에서 D-DAY 오름차순으로 정렬합니다.
  static Map<String, List<ProductExpirationItem>> groupByExpiry(
