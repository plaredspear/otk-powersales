...
/**
 * 월별 추이 막대 차트 — 목표 / 실적 / 전년 동월 3 series.
 *
 * X축은 `YYYY-MM`, Y축은 원 (천원 단위 표시는 axisLabel / tooltip 에서 처리).
...
export default function MonthlyTrendChart({ data, height = 300 }: MonthlyTrendChartProps) {
  const option = useMemo(() => {
    ...
    return {
      ...
      legend: { data: ['목표', '실적', '전년 동월'], top: 0 },
      grid: { left: 60, right: 20, top: 40, bottom: 30 },
      xAxis: { type: 'category', data: categories },
      yAxis: {
        type: 'value',
        axisLabel: {
          formatter: (v: number) => `${(v / 1000).toLocaleString()}천`,
        },
      },
      series: [
        {
