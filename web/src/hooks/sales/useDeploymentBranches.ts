...
import { fetchDeploymentBranches } from '@/api/salesBranch';
 ...
 */
export function useDeploymentBranches() {
  const userId = useAuthStore((state) => state.user?.id);
  return useQuery({
    queryKey: ['admin', 'sales', 'deployment', 'branches', userId],
    queryFn: fetchDeploymentBranches,
  });
}
