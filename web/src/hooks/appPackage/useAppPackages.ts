...
/**
 * 대규모 배포용 고정 링크(iOS 설치 페이지 + Android APK 다운로드) 조회.
 * 버전 무관 고정값이라 길게 캐시한다.
 */
export function useDistributionUrls() {
  return useQuery({
    queryKey: ['admin', 'app-packages', 'distribution-urls'] as const,
    queryFn: fetchDistributionUrls,
