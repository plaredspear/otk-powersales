...
/**
 * 제안사항(물류클레임) 첨부사진 최대 개수.
 *
 * SF `/ProposalRegist` 가 `S3ImageUniqueKey1/2` 2슬롯만 받고 레거시 화면도 "사진 (최대 2장)" 이라
 * 2 로 맞춘다. 이전 값 10 은 슬롯을 초과한 3장째부터 SF 로 전송되지 않고(조용한 누락) 공유 버킷에만
 * 사본이 남는 문제가 있었다. 모바일은 원래부터 2장.
 */
export const SUGGESTION_MAX_PHOTOS = 2;
