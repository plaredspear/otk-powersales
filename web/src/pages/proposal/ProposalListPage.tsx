...
export default function ProposalListPage() {
  ...
  const [category, setCategory] = useState<SuggestionCategory | ''>(
    () => filters.category as SuggestionCategory | '',
  );
  ...
  // SF `DKRetail__All` List View 컬럼 정합 (제안사항 번호 / 제목 / 접수사원 / 제품 /
  // 작성일자 / 제안구분 / 제안내용) + 운영 편의를 위한 조치상태 컬럼 추가.
  const columns: ColumnsType<SuggestionListItem> = [
    ...
    { title: '제안사항 번호', dataIndex: 'proposalNumber', width: 150, fixed: 'left' },
    {
      ...
      dataIndex: 'category',
      width: 160,
      render: (val: SuggestionCategory) => {
        const tag = CATEGORY_TAG[val];
        return tag ? <Tag color={tag.color}>{tag.label}</Tag> : '-';
      },
    },
    { title: '제목', dataIndex: 'title', width: 200, ellipsis: true },
    {
      title: '접수사원',
