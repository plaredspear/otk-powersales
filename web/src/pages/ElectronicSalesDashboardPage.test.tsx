...
import ElectronicSalesDashboardPage from './ElectronicSalesDashboardPage';
import * as api from '@/api/electronicSalesDashboard';
...
vi.mock('@/hooks/sales/useElectronicSalesBranches', () => ({
  useElectronicSalesBranches: () => ({
    data: [{ branchCode: 'B001', branchName: '원주1지점' }],
  }),
}));

const mockedList = vi.mocked(api.fetchList);
const mockedFilterOptions = vi.mocked(api.fetchFilterOptions);
const mockedProductLookup = vi.mocked(api.fetchProductLookup);
...
function renderPage() {
  ...
  return render(
    <QueryClientProvider client={client}>
      <ElectronicSalesDashboardPage />
    </QueryClientProvider>,
  );
}

const emptyListResponse: api.ElectronicSalesDashboardListResponse = {
...
describe('ElectronicSalesDashboardPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('조회 클릭 시 일 단위 기간 + 지점으로 조회하고 합계를 상단에 표시한다', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('전산매출 금액 합계')).toBeInTheDocument();
      expect(screen.getByText('전산매출 수량 합계')).toBeInTheDocument();
    });
  ...
  it('유통형태/거래처유형/분류 옵션이 filter-options API 로 로드된다', async () => {
    renderPage();
  ...
  it('조회 기간이 3개월을 초과하면 경고를 표시하고 조회 버튼이 비활성화된다', async () => {
    ...
    // 시작일을 오늘로부터 4개월 전으로 변경 → 3개월 초과
    const fourMonthsAgo = dayjs().subtract(4, 'month').startOf('month').format('YYYY-MM-DD');
    await user.click(startInput);
    await user.clear(startInput);
    await user.type(startInput, `${fourMonthsAgo}{Enter}`);
    // antd picker 는 blur 시 입력값을 확정하는 경로도 있어 외부 클릭으로 닫는다.
    await user.click(document.body);

    await waitFor(() => {
      expect(screen.getByText('조회 기간은 최대 3개월까지 가능합니다')).toBeInTheDocument();
    });
