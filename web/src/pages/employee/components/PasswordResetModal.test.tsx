...
describe('PasswordResetModal (Spec #582 P2-W)', () => {
  ...
  it('모달에 사번/이름 + 사번 기반 임시 비밀번호 강조 박스("100123@pwrs") + 복사 버튼이 표시된다', () => {
    ...
    expect(screen.getByText('100123@pwrs')).toBeInTheDocument();
    expect(screen.getByText('사원 본인에게 별도 전달해 주세요.')).toBeInTheDocument();
  });

  it('초기화 실행 성공 - 토스트 duration 10초 + 본문에 "100123@pwrs" 명시 + onClose 호출', async () => {
    ...
    expect(arg.description).toContain("'100123@pwrs'");
