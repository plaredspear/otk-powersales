...
import {
  useHerokuStage1CopyProgress,
  useHerokuStage1Defaults,
  useHerokuStage1Targets,
  useStartHerokuStage1Copy,
  useStartHerokuStage1CopyAll,
} from '@/hooks/admin/useHerokuMigrationStage1';
import {
  HerokuStage1AlreadyRunningError,
  type HerokuStage1CopyProgress,
  type HerokuStage1EntityResult,
  type HerokuStage1EntityStatus,
  type HerokuStage1Status,
} from '@/api/admin/herokuMigrationStage1';
import ResizableTable from '@/components/common/ResizableTable';

const { Title, Paragraph, Text } = Typography;

const STATUS_TAG: Record<HerokuStage1Status, { color: string; label: string }> = {
...
const ENTITY_STATUS_TAG: Record<HerokuStage1EntityStatus, { color: string; label: string }> = {
...
function rowsThroughput(p: HerokuStage1CopyProgress): string {
...
export default function HerokuMigrationStage1Page() {
  const progressQuery = useHerokuStage1CopyProgress();
  const targetsQuery = useHerokuStage1Targets();
  const defaultsQuery = useHerokuStage1Defaults();
  const startSingleMutation = useStartHerokuStage1Copy();
  const startBatchMutation = useStartHerokuStage1CopyAll();
  ...
  // 적재 전 대상 테이블 TRUNCATE 여부 (backend reset). 기본 true.
  ...
  const onSubmitSingle = (values: {
    ...
    startSingleMutation.mutate(
      { ...values, reset, maxRows: sampleModeToMaxRows(sampleMode) },
      {
        onError: (err) => {
  ...
  const entityColumns: ColumnsType<HerokuStage1EntityResult> = [
    {
      title: '#',
      key: 'index',
      width: 50,
    ...
    },
    {
      ...
      key: 'status',
      width: 110,
      render: (v: HerokuStage1EntityStatus) => {
    ...
    {
      title: '미매칭',
      dataIndex: 'unmatchedRows',
      key: 'unmatchedRows',
      width: 100,
      align: 'right',
      render: (v: number) =>
        v > 0 ? (
          <Text type="warning">{v.toLocaleString()}</Text>
        ) : (
  ...
  return (
    <div style={{ padding: 24 }}>
      <Title level={3}>Heroku Migration — Stage 1 S3 → COPY 적재</Title>
      <Paragraph type="secondary">
        Heroku 데이터 마이그레이션 Stage 1 (TablePlus export CSV → PostgreSQL COPY). 사용자가
        사전에 TablePlus export CSV 를 S3 에 업로드해야 함. 대용량
        (<Text code>safetycheck__workschedule__member</Text> 91만행,{' '}
        <Text code>product_favorites</Text> 38만행) 은 수 분 소요.
      </Paragraph>
      ...
      <Card style={{ marginBottom: 16 }} title="실행">
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          >
            Reset 모드 (적재 전 TRUNCATE)
          </Checkbox>
          {!reset && (
            <Text type="secondary" style={{ marginLeft: 12 }}>
              체크 해제 시 기존 row 를 비우지 않고 append 적재 — 중복/PK 충돌 주의.
            </Text>
          )}
        </div>
        ...
        {runMode === 'single' ? (
          ...
          >
            ...
            >
              <Select
                placeholder="target 선택 (예: product_favorites)"
            ...
            >
              <Input placeholder="heroku-migration/input" />
            </Form.Item>
            <Space>
              <Button
                type="primary"
        ...
        <>
          <Card title={isBatch ? '요약 (일괄)' : '요약'} style={{ marginBottom: 16 }}>
            ...
            <Space size="large" style={{ marginTop: 16 }} wrap>
              ...
              />
              <Statistic
                title="unmatched (PK 미매칭)"
                value={progress.unmatchedRows.toLocaleString()}
              />
          ...
          {isBatch && progress.entityResults.length > 0 && (
            <Card title={`Entity 별 결과 (${progress.entityResults.length})`} style={{ marginBottom: 16 }}>
              <ResizableTable<HerokuStage1EntityResult>
                rowKey="targetName"
