...
/**
 * SF 클레임 상태 picklist (DKRetail__Claim__c.Status__c) 임시 후보.
...
/**
 * SF 클레임 상태 업데이트 전송 테스트 탭 (외부 API 테스트 통합 페이지).
 *
 * UI 레이아웃은 "SF 클레임 등록" 탭과 동일한 형태(Card + 세로 Form + Row/Col)로 구성한다.
...
export default function ClaimStatusUpdateTab() {
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      <Card title="클레임 상태 업데이트 (전송 로직 추후 반영)">
        <Form form={form} layout="vertical" initialValues={{ status: 'RECEIVED' }}>
          <Row gutter={16}>
            <Col span={8}>
              <Form.Item
                label="클레임 식별자 (Claim No)"
                name="claimNo"
                rules={[{ required: true, message: '클레임 식별자는 필수입니다' }]}
              >
                <Input placeholder="SF Claim 식별자" />
              </Form.Item>
            </Col>
            <Col span={8}>
              <Form.Item
