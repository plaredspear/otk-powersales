...
import { fetchAccountsForMonthlySalesLookup, type Account } from '@/api/account';
...
/**
 * ORORA 월매출 화면의 거래처 고급 검색 모달.
 *
 * 일매출 화면의 고급 검색과 동일한 UX(검색창 1개 + 다중 컬럼 결과 그리드 + 라디오 단일 선택 + 선택 버튼)이며,
 * 백엔드는 monthly_sales_history.READ 로 가드된 `/api/v1/admin/accounts/lookup-for-monthly-sales` 를 호출한다.
 * keyword 는 거래처명/SAP코드/전화/대표자명/주소/거래처지점명 OR 매칭된다.
 */
export default function MonthlySalesAccountSearchModal({
  open,
  onClose,
  onSelect,
  initialKeyword,
  ...
  const { data, isFetching } = useQuery({
    queryKey: ['monthly-sales-account-search', submittedKeyword, page],
    queryFn: () =>
      fetchAccountsForMonthlySalesLookup({
