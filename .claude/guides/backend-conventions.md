...
- **프로퍼티 순서**: constructor 파라미터는 다음 순서로 배치한다:
  1. `@Id` (PK)
  2. `@Column` 일반 컬럼 (sfid, 데이터 필드, 플래그, 타임스탬프 등)
  3. `@ManyToOne` / `@OneToOne` 관계 필드 — `// -- Relations --` 주석으로 구분

  클래스 body의 `@OneToMany` 등 컬렉션 관계는 constructor 뒤에 위치한다.
