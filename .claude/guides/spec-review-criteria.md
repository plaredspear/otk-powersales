...
Opus 4.6 (200K 컨텍스트)가 스펙을 받고 구현을 완료하기까지의 토큰 배분:

| 단계 | 구성 요소 | 토큰 (추정) | 통제 주체 |
|------|----------|:-----------:|----------|
| 가이드 로드 | System + CLAUDE.md + impl-mode.md + conventions | ~15K | 프로젝트 설정 (고정) |
| **스펙 읽기** | **spec.md 또는 Part 파일** | **~3-12K (목표)** | **스펙 작성자** |
...
| ID | 체크 항목 | PASS | WARN | FAIL |
|----|----------|------|------|------|
| C1 | Part/스펙당 크기 | ~800줄 이하 (~12K 토큰) | 800~1200줄 (~12-18K 토큰, 구현 예산 압박) | 1200줄 초과 (~18K+ 토큰, 구현 품질 저하 위험) |
| C2 | 섹션 자기 완결성 | 각 Part 내 섹션이 독립적으로 이해 가능 | 일부 섹션이 다른 Part 참조 필요 | Part 간 순환 참조 |
| C3 | 전체 스펙 크기 | 전체 3000줄 이하 | 3000~5000줄 | 5000줄 초과 |
...
이 카테고리는 Part 파일 분할 시 적합성을 평가합니다. 단일 Part(P*.md 없음)인 경우 이 카테고리는 자동 PASS입니다.
...
| P3 | Part 순서/의존성 | 의존 순서 명확 (P1→P2→...), 단방향 의존 | 부분적으로 명확 | 순환 의존 또는 순서 불명 |
| P4 | Part 파일 크기 | 각 Part ~800줄 이하 예상 | 일부 Part 800~1200줄 예상 | Part 1200줄 초과 예상 |
| P5 | 테스트 코드 동봉 | 각 Part에 관련 테스트가 함께 포함됨 | - | 테스트가 별도 Part로 분리됨 |
...
| **과도한 크기** | 단일 Part/스펙이 1200줄 초과 | FAIL (C1) |
