...
**IMPORTANT**: 마이그레이션 파일 생성 시 버전 번호 충돌을 반드시 방지해야 합니다.

### 새 마이그레이션 파일 생성 절차

1. **현재 최대 버전 확인** (필수, 생략 금지):
```bash
...
### 금지 사항
- 기존 마이그레이션 파일의 내용 수정 금지 (이미 적용된 마이그레이션은 checksum이 고정됨)
- 버전 번호를 임의로 추정하여 생성 금지 — 반드시 `ls` 명령으로 확인 후 부여
