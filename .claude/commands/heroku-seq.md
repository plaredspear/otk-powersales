---
description: 기존소스(Heroku/Spring) 기능별 Sequence Diagram 문서 자동 생성
runOn: project
---

# Heroku/Spring Sequence Diagram 생성

기존소스(Heroku/Spring MVC + JSP + MyBatis)의 기능을 분석하여 Mermaid sequence diagram 포함 문서를 생성합니다.
...
예시: `/heroku-seq 로그인` → 기능명 = "로그인"

### 2. 모듈 매핑 확인

`docs/plan/old_source_260408/summaries/_index.md` 파일을 읽어서:
...
`heroku-analyzer` subagent(Agent tool, subagent_type="heroku-analyzer")에 분석을 위임합니다.
...
원본 소스 루트: docs/plan/old_source_260408/otg_PowerSales-master/
요약 파일 루트: docs/plan/old_source_260408/summaries/
_index.md 경로: docs/plan/old_source_260408/summaries/_index.md
...
```
docs/plan/old_source_260408/summaries/docs/<모듈명>.md
```
...
python3 -c "
import re, pathlib
text = pathlib.Path('docs/plan/old_source_260408/summaries/docs/<모듈명>.md').read_text()
...
mkdir -p docs/plan/old_source_260408/summaries/docs/attachments
npx @mermaid-js/mermaid-cli -i /tmp/<모듈명>-seq.mmd -o docs/plan/old_source_260408/summaries/docs/attachments/<모듈명>-sequence.svg -b white
```
...
`heroku-analyzer` subagent(Agent tool, subagent_type="heroku-analyzer")에 BDD 작성을 위임합니다.
...
분석 문서 경로: docs/plan/old_source_260408/summaries/docs/<모듈명>.md
...
```
✅ <기능명> 기능 분석이 완료되었습니다.
📄 분석 문서: docs/plan/old_source_260408/summaries/docs/<모듈명>.md
🖼️ SVG:      docs/plan/old_source_260408/summaries/docs/attachments/<모듈명>-sequence.svg
📋 BDD:      docs/plan/old_source_260408/summaries/docs/<모듈명>-bdd.md
```
