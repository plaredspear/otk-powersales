---
description: JPA 엔티티에 레거시 시스템(@SFObject/@SFField + @HCTable/@HCColumn) 어노테이션 적용 스펙 생성
runOn: project
---

# 레거시 Entity 어노테이션 스펙 생성
...
- **매칭 파일이 있으면**: Read 하여 Salesforce 필드 목록(필드 레이블, 필드 이름, 데이터 유형)을 파악 → SF 어노테이션 포함
- **매칭 파일이 없으면**: SF 어노테이션을 건너뛰고 다음 스텝(HC 어노테이션)으로 진행. 사용자에게 "SF Object 파일 없음 → SF 어노테이션 생략" 메시지만 출력

### 3. Heroku DB 테이블 확인

Heroku DB에 접속하여 해당 테이블의 실제 컬럼 정보를 확인합니다.
Heroku Connect 동기화 여부와 무관하게, **Heroku DB 스키마에 테이블이 존재하면** HC 어노테이션 대상으로 판단합니다.
...
- **테이블이 있으면**: 해당 테이블의 모든 컬럼을 HC 어노테이션 대상으로 포함
- **테이블이 없으면**: HC 어노테이션을 건너뜀. 사용자에게 "HC 테이블 없음 → HC 어노테이션 생략" 메시지만 출력

**SF와 HC 모두 없으면 작업을 중단합니다** (어노테이션할 대상이 없음).

### 4. 매핑 분석

확보된 소스(Entity 필드 + SF Object 정의(있는 경우) + HC 테이블 컬럼(있는 경우))를 교차 대조하여 매핑 테이블을 작성합니다.
...
SF 또는 HC가 없는 경우 해당 컬럼은 "-"로 표시합니다.

**매핑 규칙** (Account Entity를 참고):
- `@SFField` 값 = Salesforce Object 정의의 `필드 이름` 컬럼 (예: `Name`, `ABCType__c`)
- `@HCColumn` 값 = Heroku DB의 실제 컬럼명 (예: `abctype__c`, `question_num`)
- HC 시스템 컬럼(`id`, `sfid`, `isdeleted` 등)이 있는 경우 `@HCColumn`만 부여 (`@SFField` 불필요)
...
- 폴더명: `<번호>-prev-annotate-<entity명(소문자)>`
- 파일: `spec-B.md` (Backend 단일 플랫폼)

스펙 내용에 포함할 사항 (**해당 어노테이션이 있는 경우만 포함**):

#### 5-1. @SFObject / @SFField 어노테이션 적용 (SF Object 파일이 있는 경우만)
...
#### 5-2. @HCTable / @HCColumn 어노테이션 적용 (HC 테이블이 있는 경우만)
...
- 시스템 컬럼(`id`, `sfid`, `isdeleted` 등)이 있는 경우 함께 포함

#### 5-3. 체크리스트

스펙에 구현 체크리스트를 포함합니다 (해당 항목만):

- [ ] `@SFObject` 클래스 어노테이션 추가 (SF 해당 시)
- [ ] 각 필드에 `@SFField` 어노테이션 추가 (SF 해당 시)
- [ ] `@HCTable` 클래스 어노테이션 추가 (HC 해당 시)
- [ ] 각 필드에 `@HCColumn` 어노테이션 추가 (HC 해당 시)
...
```
✅ 레거시 어노테이션 스펙이 생성되었습니다.
📄 스펙: docs/specs/backlog/<번호>-prev-annotate-<entity명>/spec-B.md
