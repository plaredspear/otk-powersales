...
`docs/specs/ready/` 내 폴더 목록을 수집합니다.

**참고**: `ready/`에는 이미 승인된 스펙만 있으므로 Step 3(승인 상태 확인)은 생략 가능합니다. 다만 안전을 위해 여전히 확인합니다.

### Step 2: 각 폴더에서 스펙 파일 찾기

`docs/specs/ready/` 내 각 폴더에서 다음 우선순위로 스펙 파일을 탐색합니다 (플랫폼 무관):
...
**참고**: `ready/` 폴더에 있는 스펙은 이미 승인 완료 상태이지만, 안전을 위해 승인 이력도 확인합니다.
...
2. **모든 Part 완료 시** (모든 체크박스가 `[x]`이거나 단일 플랫폼):
   - `docs/specs/ready/<폴더>/`를 `docs/specs/completed/<폴더>/`로 이동
