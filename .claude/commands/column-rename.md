...
### 6. Flyway 마이그레이션 버전 확인

`backend/src/main/resources/db/migration/` 폴더의 최신 버전 번호를 확인하여 다음 버전 번호를 결정합니다.
...
4. **비즈니스 로직**: DB 마이그레이션 (ALTER TABLE RENAME COLUMN 목록) + 엔티티 변경 (번호 매긴 목록)
