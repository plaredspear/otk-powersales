---
description: 스펙 작성 모드 활성화 - GitHub Issue 본문 형식으로 스펙 작성
runOn: project
---
...
출력은 GitHub Issue 본문으로 사용할 수 있는 Markdown 형식으로 작성합니다.
...
1. 영문 소문자 또는 camelCase이고 모듈 인덱스에 존재 → **기존 소스 모듈명** → 소스 분석 모드
2. 그 외 자유 텍스트 → 모듈 인덱스 + controller-guide 검색
...
3. **기존 소스 분석** (legacy-analyzer agent 활용 권장):
...
3. **대화를 통한 요구사항 수집**:
   ...
   - 영향 범위 (어떤 플랫폼에 해당하는지)
   - 기술적 제약사항
4. 스펙 초안을 작성합니다 (GitHub Issue 본문 형식)
5. 사용자에게 리뷰를 요청합니다
